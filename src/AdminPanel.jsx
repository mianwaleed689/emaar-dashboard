...
export default function AdminPanel() {
  ...
  const approveVerification = async (v) => {
    if (!window.confirm(`Approve verification for ${v.name || v.email}?\n\nLevel: ${v.level || "Basic"}\n\nThis will grant verified status and send confirmation email.`)) return;
    try {
      ...
      await setDoc(doc(db, "users", v.uid), { verified: true, verifiedLevel: v.level || "basic", verifiedAt: new Date().toISOString(), kycLevel: v.level || "basic" }, { merge: true });
      await logAudit(db, { action: "kyc_approved", userId: v.uid, details: `Level: ${v.level || "basic"}` });
      // Send approval email
      if (v.email) {
        emailjs.send("service_da7nshv", "template_gl1xqhy", {
          to_email: v.email,
  ...
  const rejectVerification = async (v) => {
    ...
    if (!window.confirm(`Reject verification for ${v.name || v.email}?\n\nReason: ${rejectReason}\n\nUser will be notified to resubmit.`)) return;
    try {
      ...
      await logAudit(db, { action: "kyc_rejected", userId: v.uid, details: rejectReason });
    ...
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Outfit',sans-serif", color: T.textPrimary }}>
      ...
      <main dir={dir} className="admin-main" style={{ marginLeft: 240, minHeight: "100vh" }}>
        ...
        <div style={{ padding: "28px 28px 60px" }}>
          ...
          {tab === "digest" && (() => {
            /* ═══════════════════════════════════════════════════════════════════
               TAB 11: EMAIL DIGEST — PRO LEVEL
               Schedule, preview, send, track. EmailJS bulk send.
               Collections: digestLog, digestSchedule, digestTemplate
            ═══════════════════════════════════════════════════════════════════ */
            
            // State
            const [digestSubTab, setDigestSubTab] = React.useState("compose"); // compose | history | settings
            const [sending, setSending] = React.useState(false);
            const [testSending, setTestSending] = React.useState(false);
            const [lastResult, setLastResult] = React.useState(null);
            const [digestLog, setDigestLog] = React.useState([]);
            const [digestSchedule, setDigestSchedule] = React.useState({ enabled: true, day: "monday", hour: 8, timezone: "Asia/Dubai" });
            const [digestTemplate, setDigestTemplate] = React.useState({
            ...
            const [selectedSegment, setSelectedSegment] = React.useState("pro_all"); // pro_all | pro_only | enterprise | trial | custom
            ...
            // Load digest log from Firestore
            React.useEffect(() => {
            ...
            const avgOpenRate = 42; // Placeholder - would come from email service webhooks
