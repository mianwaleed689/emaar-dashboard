  ...
  return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:24 }}>
        {[
          { label:"Total", value:campaigns.length, color:T.gold },
          { label:"Sent", value:campaigns.filter(c=>c.status==="completed").length, color:T.green },
          { label:"Draft", value:campaigns.filter(c=>c.status==="draft"||!c.status).length, color:T.textMuted },
        ].map((s,i) => (
          <div key={i} style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:12, padding:"16px 20px" }}>
            <div style={{ fontSize:10, fontWeight:700, color:T.textMuted, textTransform:"uppercase", letterSpacing:1 }}>{s.label}</div>
            <div style={{ fontSize:28, fontWeight:900, color:s.color, fontFamily:"'Fraunces',serif", marginTop:4 }}>{s.value}</div>
          </div>
        ))}
      </div>
      </main>
