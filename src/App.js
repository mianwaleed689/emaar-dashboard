...
// Shows LandingPage to guests, Dashboard to logged-in users
function HomeRoute() {
  const [status, setStatus] = useState("loading");

  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (firebaseUser) => {
      setStatus(firebaseUser ? "auth" : "guest");
    });
    return () => unsub();
  }, []);
  ...
  if (status === "auth") return <EmaarDashboardV2 />;

  return (
    <LandingPage
      onLoginClick={() => window.location.href = "/dashboard"}
      onSignUpClick={(plan) => window.location.href = plan && plan !== "free" ? `/dashboard?plan=${plan}` : "/dashboard"}
    />
  );
}
