...
/**
 * AdminLeadsTab — Full CRM: pipeline cards, kanban, table, drawer, convert-to-user.
 *
 * Props:
 *   leads, fetchLeads, fetchUsers
 *   leadFilter, setLeadFilter
 *   leadSourceFilter, setLeadSourceFilter
 *   leadDateRange, setLeadDateRange
 *   leadSearch, setLeadSearch
 *   leadsViewMode, setLeadsViewMode
 *   showAddLead, setShowAddLead
 *   addLeadForm, setAddLeadForm
 *   addLeadLoading, setAddLeadLoading
 *   leadDrawer, setLeadDrawer
 *   leadNote, setLeadNote
 *   leadNoteSaving, setLeadNoteSaving
 *   showLossReason, setShowLossReason
 *   lossReason, setLossReason
 *   convertingLead, setConvertingLead
 *   adminUser, db
 *   setTab, setPendingOpenUid
 *   notify, logAudit, timeSince
 *   I (icons)
 */
const AdminLeadsTab = ({
  leads, fetchLeads, fetchUsers,
  ...
  const addLead = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "leads", id), {
        ...addLeadForm, status:"New", createdAt:new Date().toISOString(),
      ...
      notify("Lead added!"); setShowAddLead(false);
  ...
  const updateLeadStatus = async (leadId, newStatus, reason) => {
    try {
      ...
      if (newStatus === "Contacted" && !leads.find(l => l.id === leadId)?.respondedAt) update.respondedAt = new Date().toISOString();
      ...
      notify(`Status → ${newStatus}`); fetchLeads();
  ...
  const addNote = async () => {
    ...
    try {
      ...
      setLeadDrawer(prev => ({ ...prev, notes })); setLeadNote(""); notify("Note added");
    } catch(e) { notify("Error: " + e.message); }
  ...
  const convertToUser = async () => {
    ...
    try {
      ...
      notify("Lead converted to user!"); fetchLeads(); fetchUsers(); setLeadDrawer(null);
    } catch(e) { notify("Error: " + e.message); }
  ...
  const sendLeadEmail = async (lead, subject, body) => {
    try {
      await emailjs.send("service_da7nshv", "template_gl1xqhy", {
        to_email: lead.email, to_name: lead.name||"there",
      ...
      notify("Email sent!"); fetchLeads();
    } catch(e) { notify("Email failed: " + e.message); }
  };

  const exportLeadsCSV = () => {
    const headers = ["Name","Email","Phone","Project","Community","Source","Status","Created","Notes"];
    const rows    = filtered.map(l => [l.name||"", l.email||"", l.phone||"", l.project||"", l.community||"", l.source||"", l.status||"New", l.createdAt||"", (l.notes||[]).map(n=>n.text).join(" | ")]);
    const csv     = [headers,...rows].map(r => r.map(c => `"${String(c).replace(/"/g,'""')}"`).join(",")).join("\n");
    const a       = document.createElement("a"); a.href = URL.createObjectURL(new Blob([csv],{type:"text/csv"})); a.download = `leads_${new Date().toISOString().slice(0,10)}.csv`; a.click();
    notify(`Exported ${filtered.length} leads`);
  };

  const inputStyle = { width:"100%", padding:"10px 12px", background:T.bg, border:`1px solid ${T.border}`, borderRadius:8, color:T.white, fontSize:13, fontFamily:"'Outfit',sans-serif", boxSizing:"border-box" };
  ...
  return (
    <>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:0, borderRadius:14, background:T.surface, border:`1px solid ${T.border}`, marginBottom:20, overflow:"hidden" }}>
        <button type="button" onClick={() => { fetchLeads(); notify("Leads refreshed"); }}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:12, marginBottom:20 }}>
        {[
          ...
          <div key={s.id} onClick={() => setLeadFilter(leadFilter === s.id ? "all" : s.id)}
            style={{ padding:"16px 18px", borderRadius:12, cursor:"pointer", background:leadFilter===s.id?`${s.color}15`:T.surface, border:`1px solid ${leadFilter===s.id?s.color:T.border}`, transition:"all 0.15s" }}>
      ...
      {leadsViewMode === "kanban" && (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:12, marginBottom:20 }}>
          {["New","Contacted","Qualified","Converted","Lost"].map((stage, si) => {
            const stageColor = [T.blue,T.gold,"#8B5CF6",T.green,T.red][si];
            const stageLeads = leads.filter(l => (l.status||"New") === stage).sort((a,b) => new Date(b.createdAt||0)-new Date(a.createdAt||0));
            return (
              <div key={stage} style={{ background:T.surface, borderRadius:12, border:`1px solid ${T.border}`, overflow:"hidden" }}>
                <div style={{ padding:"12px 14px", borderBottom:`2px solid ${stageColor}`, background:`${stageColor}08` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <span style={{ fontSize:12, fontWeight:700, color:stageColor }}>{stage}</span>
                    <span style={{ fontSize:11, fontWeight:600, padding:"2px 8px", borderRadius:10, background:`${stageColor}20`, color:stageColor }}>{stageLeads.length}</span>
                  </div>
                </div>
                <div style={{ padding:8, maxHeight:400, overflowY:"auto" }}>
                  {stageLeads.length === 0
                    ...
                    : stageLeads.map(lead => (
                      <div key={lead.id} onClick={() => setLeadDrawer(lead)}
                        style={{ padding:"10px 12px", background:T.surfaceAlt, borderRadius:8, marginBottom:6, cursor:"pointer", border:`1px solid ${T.border}`, transition:"all 0.15s" }}
                        onMouseEnter={e => { e.currentTarget.style.borderColor=stageColor+"50"; e.currentTarget.style.transform="translateY(-1px)"; }}
                    ...
                    return (
                      <tr key={lead.id} style={{ borderBottom:`1px solid ${T.border}`, cursor:"pointer" }}
                        ...
                        <td style={{ padding:"12px 14px" }} onClick={e => e.stopPropagation()}>
                          <select value={lead.status||"New"} onChange={e => { if(e.target.value==="Lost") setShowLossReason(lead.id); else updateLeadStatus(lead.id,e.target.value); }}
                        ...
                        <td style={{ padding:"12px 14px" }} onClick={e => e.stopPropagation()}>
                          <div style={{ display:"flex", gap:6 }}>
                            ...
                            {lead.email && <button type="button" onClick={() => sendLeadEmail(lead)} style={{ fontSize:10, padding:"4px 8px", borderRadius:6, border:"none", background:"rgba(59,130,246,0.12)", color:T.blue, cursor:"pointer", fontWeight:600 }}>Email</button>}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(4,9,15,0.92)", zIndex:9000, display:"flex", alignItems:"center", justifyContent:"center", backdropFilter:"blur(8px)" }} onClick={() => setShowAddLead(false)}>
          <div style={{ background:T.surface, border:`1px solid ${T.green}40`, borderRadius:16, width:"95%", maxWidth:500, padding:24 }} onClick={e => e.stopPropagation()}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
              ...
              <button type="button" onClick={() => setShowAddLead(false)} style={{ background:"none", border:"none", color:T.textMuted, cursor:"pointer", fontSize:20 }}>×</button>
            </div>
            <div style={{ display:"grid", gap:12 }}>
              {[{key:"name",label:"Name",placeholder:"John Smith"},{key:"email",label:"Email",placeholder:"john@example.com",type:"email"},{key:"phone",label:"Phone",placeholder:"+971 50 123 4567"},{key:"project",label:"Interested Project",placeholder:"e.g. The Valley"}].map(f => (
                <div key={f.key}>
                  ...
                  <input type={f.type||"text"} placeholder={f.placeholder} value={addLeadForm[f.key]} onChange={e => setAddLeadForm(prev => ({ ...prev, [f.key]:e.target.value }))} style={inputStyle} />
                </div>
              ))}
              <div>
                ...
                <select value={addLeadForm.source} onChange={e => setAddLeadForm(prev => ({ ...prev, source:e.target.value }))} style={{ ...inputStyle }}>
                  {["Manual","WhatsApp","Email Inquiry","Phone Call","Walk-in","Referral","Website"].map(s => <option key={s}>{s}</option>)}
                </select>
              </div>
              <div>
                ...
                <textarea placeholder="Initial notes..." value={addLeadForm.notes} onChange={e => setAddLeadForm(prev => ({ ...prev, notes:e.target.value }))} rows={3} style={{ ...inputStyle, resize:"vertical" }} />
              </div>
            </div>
          ...
          <div style={{ background:T.surface, border:`1px solid ${T.red}40`, borderRadius:16, width:"95%", maxWidth:400, padding:24 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <select value={lossReason} onChange={e => setLossReason(e.target.value)} style={inputStyle}>
                ...
                {[["price","Too Expensive"],["competitor","Chose Competitor"],["timing","Not Ready Now"],["no_response","No Response"],["wrong_fit","Wrong Fit"],["other","Other"]].map(([v,l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </div>
        ...
        <div style={{ position:"fixed", inset:0, zIndex:8000, background:"rgba(4,9,15,0.85)", backdropFilter:"blur(4px)" }} onClick={() => setLeadDrawer(null)}>
          <div style={{ position:"absolute", right:0, top:0, bottom:0, width:"100%", maxWidth:480, background:T.surface, borderLeft:`1px solid ${T.gold}30`, display:"flex", flexDirection:"column", animation:"slideIn 0.2s ease-out" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding:"16px 24px", borderBottom:`1px solid ${T.border}`, display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
              <select value={leadDrawer.status||"New"} onChange={e => { if(e.target.value==="Lost") setShowLossReason(leadDrawer.id); else updateLeadStatus(leadDrawer.id,e.target.value); }}
              ...
              {leadDrawer.phone && <a href={`https://wa.me/${leadDrawer.phone.replace(/\D/g,"")}?text=${encodeURIComponent(`Hi ${leadDrawer.name||""}, following up on your interest in ${leadDrawer.project||"the property"}.`)}`} target="_blank" rel="noreferrer" style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, padding:"8px 14px", borderRadius:8, background:"rgba(37,211,102,0.15)", color:T.green, textDecoration:"none", fontWeight:600 }}>WhatsApp</a>}
              {leadDrawer.email && <button type="button" onClick={() => sendLeadEmail(leadDrawer)} style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, padding:"8px 14px", borderRadius:8, border:"none", background:"rgba(59,130,246,0.12)", color:T.blue, cursor:"pointer", fontWeight:600 }}>Send Email</button>}
            ...
            <div style={{ padding:"16px 24px", borderBottom:`1px solid ${T.border}` }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                {[{key:"name",label:"Name"},{key:"email",label:"Email"},{key:"phone",label:"Phone"},{key:"project",label:"Project"},{key:"community",label:"Community"},{key:"source",label:"Source"}].map(field => (
                  <div key={field.key}>
                    <label style={{ fontSize:10, color:T.textMuted, display:"block", marginBottom:4 }}>{field.label}</label>
                    <input value={leadDrawer[field.key]||""} onChange={e => setLeadDrawer(prev => ({ ...prev, [field.key]:e.target.value }))}
                      onBlur={async e => { try { await setDoc(doc(db,"leads",leadDrawer.id),{[field.key]:e.target.value,updatedAt:new Date().toISOString()},{merge:true}); notify(`${field.label} updated`); fetchLeads(); } catch{ notify("Error saving"); } }}
