...
/**
 * AdminCancellationTab — Churn analysis, exit surveys, win-back campaigns,
 * at-risk users, monthly trend chart.
 *
 * Props:
 *   users
 *   notify, I
 */
const AdminCancellationTab = ({ users, notify, I }) => {
  const now = new Date();
  ...
  const churned = users.filter(u =>
    (u.tier === "free" && u.previousTier && u.previousTier !== "free") || u.cancelledAt
  );
  const churnedThisMonth = churned.filter(u => new Date(u.cancelledAt || u.updatedAt || 0) >= thisMonth).length;
  ...
  const atRiskUsers = users.filter(u => {
    ...
    return !u.lastLoginAt || new Date(u.lastLoginAt) < fourteenAgo;
  });
  ...
  const maxChurn = Math.max(...churnTrend.map(x => x.churned));

  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>

      {/* KPI TOPBAR */}
      <div style={{ display:"flex", alignItems:"center", gap:0, borderRadius:14, background:T.surface, border:`1px solid ${T.border}`, overflow:"hidden" }}>
        <button type="button" onClick={() => notify("Cancellation data refreshed")}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        ...
        <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.border}`, padding:20 }}>
          ...
          <div style={{ fontSize:11, color:T.textMuted, marginBottom:20 }}>{reasons.reduce((s,r)=>s+r.count,0)} responses from exit surveys</div>
        ...
        <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.border}`, padding:20 }}>
            ...
            {churnTrend.map((m, i) => (
              <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
                <div style={{ display:"flex", gap:2, alignItems:"flex-end", height:120 }}>
                  <div style={{ width:16, height:Math.max(4,(m.churned/maxChurn)*120), background:T.red, borderRadius:"4px 4px 0 0", opacity:0.8 }} title={`Churned: ${m.churned}`} />
                  <div style={{ width:16, height:Math.max(4,(m.recovered/maxChurn)*120), background:T.green, borderRadius:"4px 4px 0 0", opacity:0.8 }} title={`Recovered: ${m.recovered}`} />
                </div>
          ...
          <div style={{ display:"flex", gap:20, marginTop:16 }}>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}><div style={{ width:12, height:12, background:T.red, borderRadius:3 }} /><span style={{ fontSize:11, color:T.textMuted }}>Churned</span></div>
            <div style={{ display:"flex", alignItems:"center", gap:6 }}><div style={{ width:12, height:12, background:T.green, borderRadius:3 }} /><span style={{ fontSize:11, color:T.textMuted }}>Recovered</span></div>
          </div>
        </div>
      </div>
      ...
      <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.border}`, overflow:"hidden" }}>
        ...
        {atRiskUsers.length === 0 ? (
          ...
          <div style={{ maxHeight:300, overflowY:"auto" }}>
            ...
            {atRiskUsers.length > 10 && <div style={{ padding:"12px 20px", textAlign:"center", fontSize:11, color:T.textMuted }}>+ {atRiskUsers.length-10} more at-risk users</div>}
          </div>
        )}
      </div>
