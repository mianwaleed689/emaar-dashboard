...
/**
 * AdminVerificationTab — KYC review queue + history.
 *
 * Props:
 *   verifications, fetchVerifications
 *   verifySubTab, setVerifySubTab
 *   verifyFilter, setVerifyFilter
 *   verifySearch, setVerifySearch
 *   verifyBatchMode, setVerifyBatchMode
 *   verifyBatchSelected, setVerifyBatchSelected
 *   reviewingUser, setReviewingUser
 *   rejectReason, setRejectReason
 *   approveVerification, rejectVerification
 *   users, setTab, setPendingOpenUid
 *   notify, I
 */
const AdminVerificationTab = ({
  ...
  const avgReviewHrs = reviewed.length > 0
    ? Math.round(reviewed.reduce((s, v) => s + (new Date(v.reviewedAt) - new Date(v.submittedAt)), 0) / reviewed.length / 3600000)
    : null;

  const isUrgent      = v => v.status === "pending" && v.submittedAt && (Date.now() - new Date(v.submittedAt).getTime()) > 86400000;
  const getUserCtx    = uid => users.find(u => u.uid === uid || u.id === uid);
  ...
  return (
    <>
      {/* KPI TOPBAR */}
      <div style={{ display:"flex", alignItems:"center", gap:0, borderRadius:14, background:T.surface, border:`1px solid ${T.border}`, marginBottom:20, overflow:"hidden" }}>
        <button type="button" onClick={() => { fetchVerifications(); notify("Refreshed"); }}
          style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, padding:"14px 16px", background:T.goldGlow, border:"none", borderRight:`1px solid ${T.border}`, color:T.gold, cursor:"pointer", fontFamily:"'Outfit',sans-serif", fontWeight:600, flexShrink:0 }}>
          {I.refresh}
        </button>
        {[
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:20 }}>
        {[
          { level:"Basic",        color:T.blue,  icon:"1", desc:"Email verified", features:["Email confirmation","Basic profile","Limited access"] },
          { level:"Enhanced",     color:T.gold,  icon:"2", desc:"ID + Selfie",    features:["Government ID","Selfie verification","Full project access"] },
          { level:"Institutional",color:T.green, icon:"3", desc:"Business docs",  features:["Company registration","Director ID","Enterprise features"] },
        ].map((tier, i) => (
          <div key={i} style={{ background:T.surface, borderRadius:12, padding:"16px 18px", border:`1px solid ${T.border}`, position:"relative" }}>
            <div style={{ position:"absolute", top:0, left:0, right:0, height:2, background:tier.color, opacity:0.7, borderRadius:"12px 12px 0 0" }} />
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
              {tier.features.map((f, j) => <span key={j} style={{ fontSize:10, padding:"3px 8px", borderRadius:6, background:`${tier.color}10`, color:tier.color }}>{f}</span>)}
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16 }}>
        {[{ id:"queue", label:`Review Queue (${vPending.length})` }, { id:"history", label:"History" }].map(t => (
          <button key={t.id} type="button" onClick={() => setVerifySubTab(t.id)}
            style={{ padding:"8px 16px", borderRadius:8, border:`1px solid ${verifySubTab===t.id?T.gold:T.border}`, background:verifySubTab===t.id?T.goldGlow:"transparent", color:verifySubTab===t.id?T.gold:T.textMuted, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"'Outfit',sans-serif" }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap", alignItems:"center" }}>
        ...
        <button type="button" onClick={() => { setVerifyBatchMode(!verifyBatchMode); setVerifyBatchSelected([]); }}
          ...
          {verifyBatchMode ? `✓ Batch (${verifyBatchSelected.length})` : "Batch Mode"}
        </button>
        {verifyBatchMode && verifyBatchSelected.length > 0 && (
          <button type="button" onClick={async () => {
            for (const id of verifyBatchSelected) { const v = filtered.find(x => x.id===id); if(v) await approveVerification(v); }
            setVerifyBatchSelected([]); notify(`Approved ${verifyBatchSelected.length}`);
            ...
            Approve ({verifyBatchSelected.length})
          </button>
        )}
        <div style={{ flex:1 }} />
        {vPending.filter(v => isUrgent(v)).length > 0 && (
          <div style={{ display:"flex", alignItems:"center", gap:6, padding:"6px 12px", borderRadius:8, background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.3)" }}>
            <span style={{ fontSize:11, fontWeight:700, color:T.red }}>⚠ {vPending.filter(v => isUrgent(v)).length} past SLA &gt;24h</span>
          </div>
        )}
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tr style={{ borderBottom:`2px solid ${T.border}` }}>
                  {verifyBatchMode && (
                    <th style={{ padding:"12px 10px", textAlign:"center", background:T.surfaceAlt, width:40 }}>
                      <input type="checkbox"
                        checked={verifyBatchSelected.length === filtered.filter(v=>v.status==="pending").length && filtered.filter(v=>v.status==="pending").length > 0}
                        onChange={e => e.target.checked ? setVerifyBatchSelected(filtered.filter(v=>v.status==="pending").map(v=>v.id)) : setVerifyBatchSelected([])}
              ...
              <tbody>
                {filtered.map((v) => {
                  ...
                  const waitHrs = v.submittedAt ? Math.round((Date.now() - new Date(v.submittedAt).getTime()) / 3600000) : 0;
                  const ctx     = getUserCtx(v.uid);
                  return (
                    <tr key={v.id}
                      style={{ borderBottom:`1px solid ${T.border}`, background:urgent?"rgba(239,68,68,0.03)":verifyBatchSelected.includes(v.id)?`${T.purple}08`:"transparent", cursor:"pointer" }}
                      ...
                      {verifyBatchMode && (
                        <td style={{ padding:"12px 10px", textAlign:"center" }} onClick={e => e.stopPropagation()}>
                          {v.status === "pending" && <input type="checkbox" checked={verifyBatchSelected.includes(v.id)} onChange={e => e.target.checked ? setVerifyBatchSelected([...verifyBatchSelected,v.id]) : setVerifyBatchSelected(verifyBatchSelected.filter(x=>x!==v.id))} style={{ cursor:"pointer" }} />}
                        </td>
                      )}
                      <td style={{ padding:"12px 14px" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          ...
                          <div>
                            ...
                            {ctx && <div style={{ fontSize:9, color:T.teal }}>{ctx.tier} · {ctx.createdAt ? new Date(ctx.createdAt).toLocaleDateString("en-AE",{month:"short",year:"numeric"}) : "—"}</div>}
                          </div>
                        </div>
                      </td>
                      <td style={{ padding:"12px 14px" }}>
                      ...
                      <td style={{ padding:"12px 14px" }}>
                        {v.status === "pending"
                          ? <span style={{ fontSize:11, fontWeight:600, color:urgent?T.red:waitHrs>12?T.orange:T.textMuted }}>{waitHrs}h {urgent && <span style={{ fontSize:9, padding:"2px 6px", borderRadius:4, background:"rgba(239,68,68,0.15)", color:T.red, marginLeft:4 }}>URGENT</span>}</span>
        ...
        <div style={{ position:"fixed", inset:0, zIndex:8000 }} onClick={() => { setReviewingUser(null); setRejectReason(""); }}>
          <div style={{ position:"absolute", right:0, top:0, bottom:0, width:"100%", maxWidth:540, background:T.surface, borderLeft:`1px solid ${T.gold}30`, display:"flex", flexDirection:"column", animation:"slideIn 0.2s ease-out" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding:"20px 24px", borderBottom:`1px solid ${T.border}`, display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              ...
              <button type="button" onClick={() => { setReviewingUser(null); setRejectReason(""); }} style={{ background:"none", border:"none", color:T.textMuted, cursor:"pointer", fontSize:24 }}>×</button>
            </div>

            {(() => { const ctx = getUserCtx(reviewingUser.uid); if (!ctx) return null; return (
              <div style={{ padding:"12px 24px", borderBottom:`1px solid ${T.border}`, background:T.surfaceAlt }}>
                ...
                <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}>
                  {[["Tier",ctx.tier],["Joined",ctx.createdAt?new Date(ctx.createdAt).toLocaleDateString("en-AE"):"—"],["Last Active",ctx.lastLoginAt?new Date(ctx.lastLoginAt).toLocaleDateString("en-AE"):"—"],["Country",ctx.country||"—"]].map(([l,val]) => (
                    <div key={l}><span style={{ fontSize:10, color:T.textMuted }}>{l}: </span><span style={{ fontSize:11, color:T.white, fontWeight:600 }}>{val}</span></div>
                  ))}
                </div>
              </div>
            ); })()}
            ...
            <div style={{ flex:1, overflowY:"auto", padding:24 }}>
              <div style={{ marginBottom:20 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                  {[["Full Name",reviewingUser.name||"—"],["Email",reviewingUser.email||"—"],["Phone",reviewingUser.phone||"—"],["Nationality",reviewingUser.nationality||"—"],["Date of Birth",reviewingUser.dob||"—"],["Address",reviewingUser.address||"—"]].map(([label,val]) => (
                    <div key={label} style={{ padding:"8px 12px", borderRadius:8, background:T.surfaceAlt }}>
                      <div style={{ fontSize:9, fontWeight:600, color:T.textMuted, textTransform:"uppercase", marginBottom:2 }}>{label}</div>
                      <div style={{ fontSize:12, color:T.white }}>{val}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom:20 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
                  {[{ type:"Government ID",key:"idDoc",desc:"Passport / Emirates ID" },{ type:"Selfie",key:"selfieDoc",desc:"Photo holding ID" },{ type:"Address Proof",key:"addressDoc",desc:"Utility / Bank statement" }].map((d,i) => {
                    const url = reviewingUser[d.key];
                    return (
                      <div key={i} style={{ background:T.surfaceAlt, borderRadius:10, padding:12, border:`1px solid ${url?T.green:T.border}30`, textAlign:"center" }}>
                        {url ? (
                          <a href={url} target="_blank" rel="noreferrer" style={{ display:"block" }}>
                            <img src={url} alt={d.type} style={{ width:"100%", height:80, objectFit:"cover", borderRadius:8, marginBottom:8, border:`1px solid ${T.border}` }} onError={e => { e.target.style.display="none"; }} />
                          </a>
                        ) : (
                        ...
                        <div style={{ fontSize:11, fontWeight:600, color:url?T.white:T.textMuted }}>{d.type}</div>
                        <div style={{ fontSize:9, color:T.textMuted }}>{url?"Click to view":"Not uploaded"}</div>
                      </div>
                    );
                  })}
                </div>
              ...
              {reviewingUser.reviewedAt && (
                <div style={{ padding:"12px 16px", borderRadius:10, background:`${statusColor[reviewingUser.status]}08`, border:`1px solid ${statusColor[reviewingUser.status]}25`, marginBottom:20 }}>
                  <div style={{ fontSize:11, fontWeight:600, color:statusColor[reviewingUser.status] }}>
                    {statusLabel[reviewingUser.status]} on {new Date(reviewingUser.reviewedAt).toLocaleDateString("en-AE",{day:"numeric",month:"short",year:"numeric"})}
                  </div>
              ...
              {reviewingUser.status === "pending" && (
                <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                  <div>
                    <label style={{ fontSize:10, fontWeight:600, color:T.textMuted, textTransform:"uppercase", letterSpacing:1, display:"block", marginBottom:6 }}>Rejection Reason (required)</label>
                  ...
                  <div style={{ display:"flex", gap:12 }}>
                    <button type="button" onClick={() => approveVerification(reviewingUser)}
                      style={{ flex:1, padding:"12px 0", borderRadius:10, border:"none", background:`linear-gradient(135deg,${T.green},#059669)`, color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"'Outfit',sans-serif" }}>
                      Approve
                    </button>
                    <button type="button" onClick={() => rejectVerification(reviewingUser)} disabled={!rejectReason.trim()}
                      style={{ flex:1, padding:"12px 0", borderRadius:10, border:"none", background:rejectReason.trim()?"rgba(239,68,68,0.15)":"rgba(100,116,139,0.1)", color:rejectReason.trim()?T.red:T.textMuted, fontWeight:700, fontSize:13, cursor:rejectReason.trim()?"pointer":"not-allowed", fontFamily:"'Outfit',sans-serif" }}>
                      Reject
                    </button>
                  </div>
                </div>
              ...
              {reviewingUser.uid && (
                <button type="button" onClick={() => { setTab("users"); setPendingOpenUid(reviewingUser.uid); setReviewingUser(null); }}
                  style={{ marginTop:16, width:"100%", padding:"10px", borderRadius:8, border:`1px solid ${T.gold}`, background:"transparent", color:T.gold, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"'Outfit',sans-serif" }}>
