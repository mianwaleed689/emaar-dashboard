...
/**
 * AdminTabControlTab — Feature flags, tier gating, badges, data tables.
 *
 * Props:
 *   tabSettings, setTabSettings
 *   tabSettingsSaving, setTabSettingsSaving
 *   tabSearch, setTabSearch
 *   previewTier, setPreviewTier
 *   selectedTabControl, setSelectedTabControl
 *   tabDataEdits, setTabDataEdits
 *   tabDataSaving, setTabDataSaving
 *   liveCommunityROI
 *   auditLog
 *   adminUser, db
...
const AdminTabControlTab = ({
  ...
  const getTabSetting  = key => tabSettings[key] || { visible:true, minTier:"free", badge:"none", description:"" };

  const updateTabSetting = async (tabKey, field, value) => {
    const updated = { ...tabSettings, [tabKey]: { ...getTabSetting(tabKey), [field]:value, lastModified:new Date().toISOString(), modifiedBy:adminUser?.email||"admin" } };
  ...
  const tabUsageStats = (() => {
    const stats = {};
    auditLog.forEach(log => { if (log.action==="tab_view" && log.tabKey) stats[log.tabKey]=(stats[log.tabKey]||0)+1; });
    return stats;
  })();
  ...
  const filteredTabs = ALL_TABS.filter(t => {
    ...
    return t.id.toLowerCase().includes(s) || t.category.toLowerCase().includes(s) || (setting.description||t.desc).toLowerCase().includes(s);
  });
  ...
  const updateCell      = (rowIdx, field, value) => setTabDataEdits(prev => ({ ...prev, [selectedTabControl]: getEditableRows().map((r,i) => i===rowIdx?{...r,[field]:value}:r) }));
  const addRow          = () => { if (!activeTabData) return; const empty={}; activeTabData.fields.forEach(f=>{empty[f]="";}); setTabDataEdits(prev=>({...prev,[selectedTabControl]:[...getEditableRows(),empty]})); };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.border}`, padding:"16px 20px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:12 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            {tabSettingsSaving && <div style={{ fontSize:11, color:T.green, display:"flex", alignItems:"center", gap:5 }}><div style={{ width:6, height:6, borderRadius:"50%", background:T.green }} />Saving...</div>}
            <button type="button" onClick={async () => { const snap=await getDoc(doc(db,"platformSettings","tabs")); if(snap.exists()) setTabSettings(snap.data()); notify("Refreshed"); }}
              style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, padding:"6px 12px", borderRadius:8, border:`1px solid ${T.gold}`, background:T.goldGlow, color:T.gold, cursor:"pointer", fontFamily:"'Outfit',sans-serif", fontWeight:600 }}>
              {I.refresh}
            </button>
          </div>
        </div>
        <div style={{ display:"flex", gap:12, marginTop:14, alignItems:"center", flexWrap:"wrap" }}>
          <div style={{ flex:1, minWidth:200, position:"relative" }}>
            ...
            <input type="text" placeholder="Search tabs..." value={tabSearch} onChange={e => setTabSearch(e.target.value)}
              style={{ width:"100%", padding:"10px 12px 10px 36px", borderRadius:10, border:`1px solid ${T.border}`, background:T.surfaceAlt, color:T.white, fontSize:12, fontFamily:"'Outfit',sans-serif", outline:"none" }} />
        ...
        <div style={{ display:"flex", gap:6 }}>
          {[{id:null,label:"Admin",color:T.white},{id:"free",label:"Free",color:T.textSecondary},{id:"pro",label:"Pro",color:T.gold},{id:"enterprise",label:"Ent",color:T.purple}].map(p=>(
            <button key={p.id||"admin"} type="button" onClick={() => setPreviewTier(p.id)}
              style={{ padding:"6px 12px", borderRadius:8, fontSize:10, fontWeight:600, cursor:"pointer", border:previewTier===p.id?`2px solid ${p.color}`:`1px solid ${T.border}`, background:previewTier===p.id?`${p.color}15`:"transparent", color:previewTier===p.id?p.color:T.textMuted, fontFamily:"'Outfit',sans-serif" }}>
              {p.label}
            </button>
          ))}
        </div>
      ...
      {previewTier && (
        <div style={{ background:T.surfaceAlt, borderRadius:14, border:`1px solid ${TIER_COLORS[previewTier]}30`, padding:"16px 20px" }}>
      ...
      {/* BULK ACTIONS */}
      <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.border}`, padding:"14px 20px" }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {[
            ...
            <button key={label} type="button" onClick={action}
              style={{ padding:"6px 12px", borderRadius:8, fontSize:10, fontWeight:600, cursor:"pointer", border:`1px solid ${color}40`, background:`${color}10`, color, fontFamily:"'Outfit',sans-serif" }}>
              {label}
            </button>
          ))}
        </div>
              ...
              return (
                <div key={cat}>
                  ...
                  {catTabs.map(tabObj => {
                    ...
                    return (
                      <div key={tabObj.id}
                        onClick={() => setSelectedTabControl(isSelected?null:tabObj.id)}
                        style={{ display:"grid", gridTemplateColumns:"1fr auto auto auto auto auto", alignItems:"center", gap:12, padding:"10px 20px", borderBottom:`1px solid ${T.border}`, background:isSelected?"rgba(212,168,67,0.08)":"transparent", cursor:"pointer" }}
                        ...
                        <div style={{ fontSize:9, color:T.textMuted, minWidth:70, textAlign:"right" }}>{setting.lastModified?timeSince(new Date(setting.lastModified)):"—"}</div>
                        <div onClick={e => e.stopPropagation()}>
                          <button type="button" onClick={() => updateTabSetting(tabObj.id,"visible",!isVisible)}
                            style={{ width:36, height:20, borderRadius:10, background:isVisible?T.green:"rgba(255,255,255,0.08)", border:"none", cursor:"pointer", position:"relative" }}>
                            <div style={{ position:"absolute", top:3, left:isVisible?19:3, width:14, height:14, borderRadius:"50%", background:T.white, transition:"left 0.2s" }} />
                          </button>
                        </div>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={isSelected?T.gold:T.textMuted} strokeWidth="2" style={{ transform:isSelected?"rotate(90deg)":"none", transition:"transform 0.2s", flexShrink:0 }}><path d="m9 18 6-6-6-6"/></svg>
                      </div>
                    );
                  })}
                </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.gold}40`, padding:"16px 20px" }}>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                {TIERS.map(tier => {
                  ...
                  return <button key={tier} type="button" onClick={() => updateTabSetting(selectedTabControl,"minTier",tier)}
                    style={{ flex:1, padding:"10px 0", borderRadius:10, fontSize:12, fontWeight:700, cursor:"pointer", border:`2px solid ${minTier===tier?TIER_COLORS[tier]:T.border}`, background:minTier===tier?`${TIER_COLORS[tier]}18`:T.surfaceAlt, color:minTier===tier?TIER_COLORS[tier]:T.textMuted, fontFamily:"'Outfit',sans-serif" }}>
                    {TIER_LABELS[tier]}
                  </button>;
                })}
              </div>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                {Object.entries(BADGE_TYPES).map(([key,val]) => {
                  const current = getTabSetting(selectedTabControl).badge||"none";
                  const color   = val?val.color:T.textMuted;
                  return <button key={key} type="button" onClick={() => updateTabSetting(selectedTabControl,"badge",key)}
                    style={{ padding:"8px 16px", borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer", border:`2px solid ${current===key?color:T.border}`, background:current===key?`${color}15`:T.surfaceAlt, color:current===key?color:T.textMuted, fontFamily:"'Outfit',sans-serif" }}>
                    {key==="none"?"None":val.label}
                  </button>;
                })}
              </div>
            </div>
            <div>
              ...
              <input type="text" placeholder={ALL_TABS.find(t=>t.id===selectedTabControl)?.desc||"Tab description..."}
                value={getTabSetting(selectedTabControl).description||""} onChange={e => updateTabSetting(selectedTabControl,"description",e.target.value)}
                style={{ width:"100%", padding:"10px 14px", borderRadius:10, border:`1px solid ${T.border}`, background:T.surfaceAlt, color:T.white, fontSize:12, fontFamily:"'Outfit',sans-serif", outline:"none" }} />
            </div>
          </div>
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                  <thead>
                    <tr style={{ background:T.surfaceAlt }}>
                      {activeTabData.fields.map(f=><th key={f} style={{ padding:"10px 12px", textAlign:"left", fontSize:10, fontWeight:700, color:T.textMuted, textTransform:"uppercase", borderBottom:`1px solid ${T.border}` }}>{activeTabData.labels[f]}</th>)}
