...
/**
 * AdminAuditLogTab — Audit log viewer with 30-day activity chart, retention
 * policy, webhook/SIEM integration, alert thresholds, and REST API panel.
 *
 * Props:
 ...
 *   auditWebhookUrl,    setAuditWebhookUrl
 *   auditAlertThr,      setAuditAlertThr
 *   auditRetentionSaved, setAuditRetentionSaved
 *   auditWebhookSaved,   setAuditWebhookSaved
 *   auditAlertSaved,     setAuditAlertSaved
 *   apiKeyLabel, setApiKeyLabel
 *   apiKeys,     setApiKeys
 *   newApiKey,   setNewApiKey
 *   apiKeyGenerating, setApiKeyGenerating
 *   apiKeyCopied, setApiKeyCopied
 *   setAuditWebhook, setAlertThreshold
 *   adminUser, db
 *   DataCalendar, UpdateChecklist, AuditLogTable
 *   I (icons), now, API_BASE, logAudit
 */
const AdminAuditLogTab = ({
  ...
  const last7 = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      count: auditLog.filter(l => { try { return new Date(l.changedAt).toDateString() === day; } catch { return false; } }).length,
    };
  });
  ...
  return (
    <>
      {/* ══ STATS TOPBAR ══ */}
      <div style={{ display:"flex", alignItems:"center", gap:0, borderRadius:14, background:T.surface, border:`1px solid ${T.border}`, marginBottom:24, overflow:"hidden" }}>
        <button type="button" onClick={fetchAuditLog}
          style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, padding:"10px 14px", background:T.goldGlow, border:"none", borderRight:`1px solid ${T.border}`, color:T.gold, cursor:"pointer", fontFamily:"'Outfit',sans-serif", fontWeight:600, flexShrink:0 }}>
          {I.refresh}
        </button>
      ...
      <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.border}`, marginBottom:24, padding:"16px 20px 12px" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
          ...
          <div style={{ fontSize:10, color:T.textMuted }}>Peak: <span style={{ color:T.gold, fontWeight:700 }}>{max30} events/day</span></div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14 }}>
          ...
          <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.border}`, padding:"16px 18px" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
              <input type="range" min={1} max={50} value={auditAlertThr} onChange={e => setAuditAlertThr(parseInt(e.target.value))} style={{ flex:1, accentColor:T.gold }} />
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
          ...
          <div style={{ background:T.surface, borderRadius:14, border:`1px solid ${T.gold}30`, padding:"18px 20px" }}>
            ...
            <button type="button" disabled={apiKeyGenerating} onClick={async () => {
              ...
              } catch(e) { /* notify handled by parent */ }
