/* ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢ââ€¢
   DXB ANALYTICS ââ‚¬â€ PROJECT DATA AUDIT HELPER
   ...
   USAGE:
     ...
     // â†â€™ [{ severity: "error", field: "status", msg: "..." }, ...]

     // Entire catalog
     const report = auditCatalog(SEED_PROJECTS);
     // â†â€™ { totalProjects, byRaised, topIssues, perProject }
...
/* ââ€¢ââ€¢ââ€¢ INDIVIDUAL AUDIT RULES ââ€¢ââ€¢ââ€¢ */

const RULES = [
  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ RERA format ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
    id: "rera-format",
    severity: "error",
    check: (p) => {
      ...
      if (s.length > 6) return `RERA number "${s}" is ${s.length} digits ââ‚¬â€ real RERA project numbers are 3-6 digits. Likely a DLD unit/listing ID.`;
      if (/^(\d)\1+$/.test(s)) return `RERA number "${s}" contains only repeating digits ââ‚¬â€ likely placeholder.`;
      if (/^(1234|5678|0000|9999)/.test(s)) return `RERA number "${s}" looks like a test placeholder.`;
      return null;
    },
  },

  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ Status vs construction % mismatch ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
    id: "status-vs-pct",
    severity: "error",
    check: (p) => {
  ...
  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ Lifecycle vs construction % mismatch ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
  ...
  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ Golden Visa eligibility check ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
    id: "golden-visa-wrong",
    severity: "warning",
    check: (p) => {
      if (p.priceMin >= GOLDEN_VISA_THRESHOLD && p.goldenVisa === false) {
        return `priceMin is AED ${(p.priceMin / 1000000).toFixed(2)}M (â‰¥ threshold) but goldenVisa=false. Should be true.`;
      }
      /* Partial ââ‚¬â€ 1BR below threshold, larger units above */
      if (p.unitBreakdown?.length > 0) {
        ...
        if (someAbove && p.goldenVisa === false) {
          return `Some unit types have priceFrom â‰¥ AED 2M (Golden Visa threshold) but goldenVisa=false.`;
        }
      }
      return null;
    },
  ...
  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ Yield sanity vs community benchmark ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
    id: "yield-outlier",
    severity: "warning",
    check: (p) => {
  ...
  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ Unit breakdown totals vs totalUnits ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
  ...
  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ PPSF vs priceMin/sqft sanity ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
    id: "ppsf-sanity",
    severity: "info",
    check: (p) => {
  ...
  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ Payment plan label sanity ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
  ...
  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ Required fields ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
  ...
  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ Data staleness ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
    id: "stale-data",
    severity: "info",
    check: (p) => {
      const verified = p._audit?.lastVerified;
      if (!verified) return `No _audit.lastVerified timestamp ââ‚¬â€ verification status unknown.`;
      const days = (Date.now() - new Date(verified)) / 86400000;
      if (days > 180) return `Data last verified ${Math.round(days)} days ago ââ‚¬â€ consider re-verifying.`;
      return null;
    },
  },

  /* ââ€â‚¬ââ€â‚¬ââ€â‚¬ Distance range sanity ââ€â‚¬ââ€â‚¬ââ€â‚¬ */
  {
    id: "distance-sanity",
    severity: "warning",
    check: (p) => {
      ...
      for (const [field, max] of checks) {
        if (p[field] != null && (p[field] < 0 || p[field] > max)) {
          return `${field}=${p[field]}km is outside plausible range (0ââ‚¬â€œ${max}km).`;
        }
      }
      return null;
    },
...
export function auditCatalog(projects) {
  ...
  /* Top offenders ââ‚¬â€ sorted by error count descending */
...
export function printAuditReport(report) {
  console.log(`\nââ€ââ€ââ€ DXB ANALYTICS ââ‚¬â€ CATALOG AUDIT ââ€ââ€ââ€`);
  ...
  console.log(`ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€ââ€\n`);
}
