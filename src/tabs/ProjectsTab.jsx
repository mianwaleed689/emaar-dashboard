              ...
              if (isDld) {
                return (
                  <div style={{ width:52, height:52, borderRadius:"50%", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", border:`2px solid ${T.teal}`, background:"rgba(20,184,166,0.15)", flexShrink:0 }}>
                    <span style={{ fontSize:16, color:T.teal, lineHeight:1 }}>�S</span>
                ...
                <div className="chart-box" style={{ padding:0, overflow:"hidden", cursor:"pointer", position:"relative" }}
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
                      <div style={{ flex:1, paddingRight:70 /* room for DLD Verified badge */ }}>
                        ...
                        <div style={{ display:"flex", gap:5, flexWrap:"wrap", marginTop:6 }}>
                          ...
                          {p.escrowBank && <span style={{ fontSize:9, padding:"2px 7px", borderRadius:5, background:"rgba(20,184,166,0.08)", color:T.teal, fontWeight:700 }}>Escrow �S</span>}
                        </div>
                      </div>
                  ...
                  {(() => {
                    ...
                    <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                      {typeof p.distMetro === "number" && p.distMetro > 0 && <span style={{ fontSize:10, padding:"2px 7px", borderRadius:8, background:p.distMetro <= 0.8 ? "rgba(16,185,129,0.15)" : T.surfaceAlt, color:p.distMetro <= 0.8 ? T.green : T.textMuted }}>Metro {p.distMetro <= 0.8 ? "�0�800m" : p.distMetro + "km"}</span>}
              ...
              <div style={{ animation:"fadeUp 0.4s ease-out forwards" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, background:"rgba(20,184,166,0.04)", border:`1px solid ${T.border}`, marginBottom:14, flexWrap:"wrap" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <span style={{ fontSize:12, color:T.teal, fontWeight:800 }}>�S</span>
...
{projDetailTab === "report" && (
        ...
        const units = selectedProject.unitBreakdown && typeof selectedProject.unitBreakdown === "object" ? Object.entries(selectedProject.unitBreakdown).map(([type,count]) => type + ": " + count + " units").join(", ") : "";
                      ...
                      const txt = [
                        "�x�"️ DXB ANALYTICS  PROPERTY DATA REPORT",
                        "������������������������������������������������",
                        `�xR ${selectedProject.project || selectedProject.name}`,
                        `�x�� Developer: ${selectedProject.developer}`,
                        `�x� Community: ${selectedProject.community}`,
                        `�x�� Type: ${selectedProject.type}`,
                        "",
                        "�x� PRICING",
                        ...
                        "",
                        `�x� RERA: ${selectedProject.reraNo||selectedProject.projectNumber||"TBC"} | Escrow: ${selectedProject.escrowBank||"TBC"}`,
                        "",
                        `�x Full report: ${projectUrl}`,
                        "������������������������������������������������",
                      ...
                      return (
                        <div className="chart-box" style={{ padding:18, marginBottom:12 }}>
                          ...
                          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                            <button type="button" onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(txt)}`,"_blank")} style={btnStyle("37,211,102")}>�x� WhatsApp</button>
                            <button type="button" onClick={() => window.open(`mailto:?subject=${encodeURIComponent(emailSubject)}&body=${encodeURIComponent(txt)}`,"_blank")} style={btnStyle("59,130,246")}>�S0️ Email</button>
                            <button type="button" onClick={async () => {
                              try {
                                await navigator.clipboard.writeText(projectUrl);
                                const el = document.activeElement;
                                const original = el && el.textContent;
                                if (el && el.textContent != null) { el.textContent = "�S Copied!"; setTimeout(() => { if (el && original) el.textContent = original; }, 1500); }
                              } catch {}
                            }} style={btnStyle("212,168,67")}>�x Copy Link</button>
