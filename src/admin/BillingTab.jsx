...
const BillingTab = ({ db, T, notify, users, adminUser }) => {
  ...
  // MRR projection â€” last 6 months + 3 projected
  ...
  // Upcoming renewals â€” users whose subscription expires in 7 days
  ...
  // Churn predictor â€” paid users inactive 14+ days + expiring soon
  ...
  const generateInvoice = (payment, user) => {
    const inv = {
      ...
      date:        payment.createdAt ? new Date(payment.createdAt).toLocaleDateString("en-AE") : "â€”",
      dueDate:     payment.createdAt ? new Date(new Date(payment.createdAt).getTime() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString("en-AE") : "â€”",
      customer:    user?.email || payment.userEmail || "â€”",
      plan:        payment.plan || payment.tier || "Pro",
      amount:      payment.amount || PRICES[payment.tier] || 99,
      status:      payment.status || "succeeded",
      stripeId:    payment.stripePaymentId || payment.stripeId || "â€”",
    };
  ...
  // Retry failed payment â€” mark as retry_requested in Firestore
  const retryPayment = async (payment) => {
    ...
    try {
      ...
      await addDoc(collection(db, "adminAlerts"), {
        message: `ðŸ”„ Payment retry requested for ${payment.userEmail || "user"} â€” AED ${payment.amount || 0}`,
        type: "payment_retry", severity: "warning", read: false,
        createdAt: new Date().toISOString(), source: "admin/billing",
      });
      notify("âœ… Retry flagged â€” contact user to update payment method", "success");
    } catch(e) {
  ...
  return (
    <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {upcomingRenewals.length > 0 && (
            <span style={{ fontSize: 11, padding: "4px 12px", borderRadius: 8, background: "rgba(212,168,67,0.1)", color: T.gold, fontWeight: 700, border: `1px solid ${T.gold}33` }}>
              ðŸ”” {upcomingRenewals.length} Renewal{upcomingRenewals.length > 1 ? "s" : ""} due
            </span>
          )}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <SubTab id="overview"  label="ðŸ“Š Revenue Overview" />
        <SubTab id="payments"  label="ðŸ’³ Payment History" />
        <SubTab id="renewals"  label={`ðŸ”” Renewals (${upcomingRenewals.length})`} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              {[
                ...
                <div key={p.label} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, color: p.color, width: 90, textAlign: "right" }}>
                    {p.rev > 0 ? `AED ${p.rev.toLocaleString()}` : "â€”"}
                  </div>
                </div>
              ))}
            ...
            <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
              ...
              {[
                ...
                { label: "Avg Revenue Per User", value: breakdown.pro + breakdown.ent > 0 ? `AED ${Math.round(mrr / (breakdown.pro + breakdown.ent))}` : "â€”", color: T.teal },
                { label: "Trial â†’ Paid Conv. Rate", value: users.filter(u => u.tier === "pro_trial").length > 0 ? `${Math.round((breakdown.pro / (breakdown.pro + users.filter(u=>u.tier==="pro_trial").length)) * 100)}%` : "â€”", color: T.green },
                { label: "Paying User %", value: users.length > 0 ? `${Math.round(((breakdown.pro + breakdown.ent) / users.length) * 100)}%` : "â€”", color: T.blue },
              ...
              {payments.map(p => {
                ...
                return (
                  <div key={p.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 1fr", gap: 8, padding: "10px 12px", borderRadius: 8, background: T.surfaceAlt, border: `1px solid ${p.status === "failed" ? T.red + "33" : T.border}`, alignItems: "center" }}>
                    <div style={{ fontSize: 12, color: T.white, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.userEmail || user?.email || "â€”"}</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: T.gold }}>AED {p.amount || PRICES[p.tier] || "â€”"}</div>
                    <div style={{ fontSize: 11, color: T.textSecondary }}>{p.plan || p.tier || "Pro"}</div>
                    <div><span style={{ fontSize: 10, fontWeight: 700, color: statusColor, padding: "2px 8px", borderRadius: 6, background: statusColor + "12", border: `1px solid ${statusColor}33` }}>{p.status || "â€”"}</span></div>
                    <div style={{ fontSize: 11, color: T.textMuted }}>{p.createdAt ? new Date(p.createdAt).toLocaleDateString("en-AE") : "â€”"}</div>
      ...
      {tab === "renewals" && (
        <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
          <div style={{ fontFamily: "'Fraunces',serif", fontSize: 16, fontWeight: 700, color: T.white, marginBottom: 4 }}>Upcoming Renewals â€” Next 7 Days</div>
          <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 16 }}>Users whose subscription expires within 7 days â€” reach out proactively to reduce churn</div>
          {upcomingRenewals.length === 0 ? (
            <div style={{ color: T.textMuted, fontSize: 13, textAlign: "center", padding: "40px 0" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {upcomingRenewals.map(u => (
                <div key={u.uid} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", borderRadius: 10, background: T.surfaceAlt, border: `1px solid ${u.daysUntilRenewal <= 2 ? T.red + "44" : T.border}` }}>
                  ...
                  <button type="button"
                    onClick={() => { notify(`ðŸ“§ Send renewal reminder to ${u.email}`, "info"); }}
      ...
      {/* â”€â”€ CHURN RISK TAB â”€â”€ */}
      {tab === "churn" && (
        <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
          <div style={{ fontFamily: "'Fraunces',serif", fontSize: 16, fontWeight: 700, color: T.white, marginBottom: 4 }}>Churn Risk Predictor</div>
          <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 16 }}>Paid users inactive 14+ days AND subscription expiring within 30 days â€” highest churn probability</div>
          {churnRisk.length === 0 ? (
            <div style={{ color: T.textMuted, fontSize: 13, textAlign: "center", padding: "40px 0" }}>
              ...
              {churnRisk.map(u => (
                <div key={u.uid} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", borderRadius: 10, background: T.surfaceAlt, border: `1px solid ${u.riskScore >= 80 ? T.red + "44" : u.riskScore >= 50 ? T.orange + "44" : T.border}` }}>
                  ...
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button type="button"
                      onClick={() => notify(`ðŸ“§ Win-back email queued for ${u.email}`, "info")}
                    ...
                    <button type="button"
                      onClick={() => notify(`ðŸŽ 1 week extension offered to ${u.email}`, "success")}
...
/* â”€â”€â”€ S17: REFERRAL PROGRAM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
   Admin tab "referral":
   - Referral link generator per user
   - Track clicks â†’ signups â†’ conversions
