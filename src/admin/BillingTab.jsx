...
const BillingTab = ({ db, T, notify, users, adminUser }) => {
  ...
  // Tier pricing
  const PRICES = { pro: PRICING.pro, enterprise: PRICING.enterprise, pro_trial: 0, free: 0 };
  ...
  // MRR projection ââ‚¬â€ last 6 months + 3 projected
  ...
  // Upcoming renewals ââ‚¬â€ users whose subscription expires in 7 days
  ...
  // Churn predictor ââ‚¬â€ paid users inactive 14+ days + expiring soon
  ...
  const generateInvoice = (payment, user) => {
    const inv = {
      ...
      date:        payment.createdAt ? new Date(payment.createdAt).toLocaleDateString("en-AE") : "ââ‚¬â€",
      dueDate:     payment.createdAt ? new Date(new Date(payment.createdAt).getTime() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString("en-AE") : "ââ‚¬â€",
      customer:    user?.email || payment.userEmail || "ââ‚¬â€",
      plan:        payment.plan || payment.tier || "Pro",
      amount:      payment.amount || PRICES[payment.tier] || 99,
      status:      payment.status || "succeeded",
      stripeId:    payment.stripePaymentId || payment.stripeId || "ââ‚¬â€",
    };
    ...
    <div class="header"><div><div class="logo">DXB Analytics</div><div class="sub">The Address Holding Â· Dubai, UAE<br>info@theaddressholding.ae</div></div>
    ...
    <div class="footer">DXB Analytics Â· Dubai Real Estate Intelligence Â· emaar-dashboard.vercel.app<br>For billing inquiries: info@theaddressholding.ae</div>
    </body></html>`;
  ...
  // Retry failed payment ââ‚¬â€ mark as retry_requested in Firestore
  const retryPayment = async (payment) => {
    ...
    try {
      ...
      await addDoc(collection(db, "adminAlerts"), {
        message: `ğŸâ€„ Payment retry requested for ${payment.userEmail || "user"} ââ‚¬â€ AED ${payment.amount || 0}`,
        type: "payment_retry", severity: "warning", read: false,
        createdAt: new Date().toISOString(), source: "admin/billing",
      });
      notify("âœâ€¦ Retry flagged ââ‚¬â€ contact user to update payment method", "success");
    } catch(e) {
      notify("âŒ " + e.message, "error");
    ...
    <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: T.textMuted, fontSize: 13 }}>Payment history Â· Invoice engine Â· Renewal alerts Â· Churn predictor</p>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {failedPayments.length > 0 && (
            <span style={{ fontSize: 11, padding: "4px 12px", borderRadius: 8, background: "rgba(239,68,68,0.12)", color: T.red, fontWeight: 700, border: `1px solid rgba(239,68,68,0.2)` }}>
              âš ï¸ {failedPayments.length} Failed Payment{failedPayments.length > 1 ? "s" : ""}
            </span>
          )}
          {upcomingRenewals.length > 0 && (
            <span style={{ fontSize: 11, padding: "4px 12px", borderRadius: 8, background: "rgba(212,168,67,0.1)", color: T.gold, fontWeight: 700, border: `1px solid ${T.gold}33` }}>
              ğŸâ€â€ {upcomingRenewals.length} Renewal{upcomingRenewals.length > 1 ? "s" : ""} due
            </span>
          )}
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        ...
        <KpiCard label="Paying Users" value={breakdown.pro + breakdown.ent} color={T.green} sub={`${breakdown.pro} Pro Â· ${breakdown.ent} Enterprise`} />
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <SubTab id="overview"  label="ğŸâ€œŠ Revenue Overview" />
        <SubTab id="payments"  label="ğŸâ€™³ Payment History" />
        <SubTab id="renewals"  label={`ğŸâ€â€ Renewals (${upcomingRenewals.length})`} />
        <SubTab id="churn"     label={`âš ï¸ Churn Risk (${churnRisk.length})`} />
      </div>

      {/* ââ€â‚¬ââ€â‚¬ OVERVIEW TAB ââ€â‚¬ââ€â‚¬ */}
      {tab === "overview" && (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              {[
                ...
                <div key={p.label} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, color: p.color, width: 90, textAlign: "right" }}>
                    {p.rev > 0 ? `AED ${p.rev.toLocaleString()}` : "ââ‚¬â€"}
                  </div>
                </div>
              ))}
            ...
            <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
              ...
              {[
                ...
                { label: "Avg Revenue Per User", value: breakdown.pro + breakdown.ent > 0 ? `AED ${Math.round(mrr / (breakdown.pro + breakdown.ent))}` : "ââ‚¬â€", color: T.teal },
                { label: "Trial â†â€™ Paid Conv. Rate", value: users.filter(u => u.tier === "pro_trial").length > 0 ? `${Math.round((breakdown.pro / (breakdown.pro + users.filter(u=>u.tier==="pro_trial").length)) * 100)}%` : "ââ‚¬â€", color: T.green },
                { label: "Paying User %", value: users.length > 0 ? `${Math.round(((breakdown.pro + breakdown.ent) / users.length) * 100)}%` : "ââ‚¬â€", color: T.blue },
      ...
      {/* ââ€â‚¬ââ€â‚¬ PAYMENT HISTORY TAB ââ€â‚¬ââ€â‚¬ */}
      {tab === "payments" && (
        <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div style={{ fontFamily: "'Fraunces',serif", fontSize: 16, fontWeight: 700, color: T.white }}>Payment History</div>
            <div style={{ fontSize: 12, color: T.textMuted }}>{payments.length} records Â· Firestore payments collection</div>
          </div>
          {loading ? (
            ...
            <div style={{ color: T.textMuted, fontSize: 13, textAlign: "center", padding: "40px 0" }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>ğŸâ€™³</div>
              ...
              {payments.map(p => {
                ...
                return (
                  <div key={p.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 1fr", gap: 8, padding: "10px 12px", borderRadius: 8, background: T.surfaceAlt, border: `1px solid ${p.status === "failed" ? T.red + "33" : T.border}`, alignItems: "center" }}>
                    <div style={{ fontSize: 12, color: T.white, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.userEmail || user?.email || "ââ‚¬â€"}</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: T.gold }}>AED {p.amount || PRICES[p.tier] || "ââ‚¬â€"}</div>
                    <div style={{ fontSize: 11, color: T.textSecondary }}>{p.plan || p.tier || "Pro"}</div>
                    <div><span style={{ fontSize: 10, fontWeight: 700, color: statusColor, padding: "2px 8px", borderRadius: 6, background: statusColor + "12", border: `1px solid ${statusColor}33` }}>{p.status || "ââ‚¬â€"}</span></div>
                    <div style={{ fontSize: 11, color: T.textMuted }}>{p.createdAt ? new Date(p.createdAt).toLocaleDateString("en-AE") : "ââ‚¬â€"}</div>
      ...
      {/* ââ€â‚¬ââ€â‚¬ RENEWALS TAB ââ€â‚¬ââ€â‚¬ */}
      {tab === "renewals" && (
        <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
          <div style={{ fontFamily: "'Fraunces',serif", fontSize: 16, fontWeight: 700, color: T.white, marginBottom: 4 }}>Upcoming Renewals ââ‚¬â€ Next 7 Days</div>
          <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 16 }}>Users whose subscription expires within 7 days ââ‚¬â€ reach out proactively to reduce churn</div>
          {upcomingRenewals.length === 0 ? (
            <div style={{ color: T.textMuted, fontSize: 13, textAlign: "center", padding: "40px 0" }}>
              ...
              {upcomingRenewals.map(u => (
                <div key={u.uid} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", borderRadius: 10, background: T.surfaceAlt, border: `1px solid ${u.daysUntilRenewal <= 2 ? T.red + "44" : T.border}` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: T.textMuted }}>{u.tier} Â· Expires {new Date(u.subscriptionExpiry).toLocaleDateString("en-AE")}</div>
                  </div>
                  ...
                  <button type="button"
                    onClick={() => { notify(`ğŸâ€œ§ Send renewal reminder to ${u.email}`, "info"); }}
      ...
      {/* ââ€â‚¬ââ€â‚¬ CHURN RISK TAB ââ€â‚¬ââ€â‚¬ */}
      {tab === "churn" && (
        <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
          <div style={{ fontFamily: "'Fraunces',serif", fontSize: 16, fontWeight: 700, color: T.white, marginBottom: 4 }}>Churn Risk Predictor</div>
          <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 16 }}>Paid users inactive 14+ days AND subscription expiring within 30 days ââ‚¬â€ highest churn probability</div>
          {churnRisk.length === 0 ? (
            <div style={{ color: T.textMuted, fontSize: 13, textAlign: "center", padding: "40px 0" }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>âœâ€¦</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {churnRisk.map(u => (
                <div key={u.uid} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", borderRadius: 10, background: T.surfaceAlt, border: `1px solid ${u.riskScore >= 80 ? T.red + "44" : u.riskScore >= 50 ? T.orange + "44" : T.border}` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: T.white }}>{u.email}</div>
                    <div style={{ fontSize: 11, color: T.textMuted }}>{u.tier} Â· Last active {u.daysSinceActive}d ago Â· Expires in {u.daysToExpiry}d</div>
                  </div>
                  {/* Risk score bar */}
                  ...
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button type="button"
                      onClick={() => notify(`ğŸâ€œ§ Win-back email queued for ${u.email}`, "info")}
                    ...
                    <button type="button"
                      onClick={() => notify(`ğŸ 1 week extension offered to ${u.email}`, "success")}
...
/* ââ€â‚¬ââ€â‚¬ââ€â‚¬ S17: REFERRAL PROGRAM ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬ââ€â‚¬
   Admin tab "referral":
   - Referral link generator per user
   - Track clicks â†â€™ signups â†â€™ conversions
