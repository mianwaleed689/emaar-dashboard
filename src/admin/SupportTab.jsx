...
function SupportTab({ T, I, db, notify, adminUser, users, setTab, setPendingOpenUid }) {
  ...
  const [chatSettings, setChatSettings] = useState({
    ...
    autoAccept: false,
    maxConcurrent: 3,
    welcomeMessage: "Hi! ‚‚Ç¨¢ How can we help you today?",
  ...
  const categories = [
    { id: "bug", label: "Bug Report", color: T.red, icon: "üâ¢" },
    { id: "data", label: "Data Question", color: T.orange, icon: "ü‚Äúà" },
    { id: "feature", label: "Feature Request", color: T.purple, icon: "‚úø" },
    { id: "billing", label: "Billing Query", color: T.green, icon: "‚‚Ç¨¢" },
    { id: "account", label: "Account Issue", color: T.blue, icon: "‚‚Ç¨¢" },
    { id: "other", label: "Other", color: T.textMuted, icon: "‚‚Ç¨¢" },
  ];
  ...
  useEffect(() => {
    const fetchAutomation = async () => {
      try {
        ...
        setWorkflowTriggers([
          { id: "wf_1", name: "Resolved ‚Ü‚Äô Add Tag", trigger: "status_change", triggerValue: "resolved", actions: [{ type: "add_tag", value: "resolved" }], enabled: true },
          ...
          try {
            const escalationUpdate = {
              ...
              internalNotes: [
                ...
                { text: `‚ö°Ô∏è AUTO-ESCALATED: SLA breached (>${slaSettings.defaultHours}h without resolution)`, by: "System", at: now.toISOString(), isSystem: true }
              ],
  ...
  const handleFileUpload = async (e, isInternalNote = false) => {
    ...
    try {
      ...
      if (isInternalNote) {
        ...
        const newNote = {
          text: `ü‚ÄúÑ Attached file: ${file.name}`,
          by: adminUser?.email || "admin",
          at: new Date().toISOString(),
          attachment
        };
...
DXB Analytics Support`,
  ...
  const webhookEvents = [
    { id: "ticket_created", label: "Ticket Created", icon: "‚‚Ç¨¢" },
    { id: "ticket_resolved", label: "Ticket Resolved", icon: "" },
    { id: "ticket_assigned", label: "Ticket Assigned", icon: "‚‚Ç¨¢" },
    { id: "sla_breach", label: "SLA Breached", icon: "‚‚Äì‚Äò" },
    { id: "reply_sent", label: "Reply Sent", icon: "ü‚Äôº" },
    { id: "priority_changed", label: "Priority Changed", icon: "‚‚Ç¨¢" },
  ];
...
");
      ...
      if (matches) {
        try {
          const assignUpdate = {
            ...
            internalNotes: [
              ...(ticket.internalNotes || []),
              { text: `ü‚Äòõ Auto-assigned to ${rule.assignToName} by rule: "${rule.name}"`, by: "System", at: new Date().toISOString(), isSystem: true }
            ],
            updatedAt: new Date().toISOString()
          };
      ...
      for (const action of workflow.actions) {
        switch (action.type) {
          ...
          case "set_status":
            ...
            break;
          case "add_note":
            newNotes.push({ text: `ü‚Äòõ ${action.value}`, by: "System", at: new Date().toISOString(), isSystem: true });
            break;
  ...
  const kbCategories = [
    { id: "getting-started", label: "Getting Started", icon: "ü‚Äùá" },
    { id: "billing", label: "Billing & Payments", icon: "‚‚Ç¨¢" },
    { id: "technical", label: "Technical Issues", icon: "‚‚Ç¨¢" },
    { id: "features", label: "Features & Usage", icon: "‚úø" },
    { id: "account", label: "Account & Security", icon: "ü‚Äîâ" },
  ];
  ...
  const analyzeSentiment = (ticket) => {
    if (!ticket) return { sentiment: "neutral", emoji: "üüâ", color: T.textMuted, label: "Neutral" };
    ...
    // Determine sentiment
    if (scores.urgent >= 3) return { sentiment: "urgent", emoji: "‚‚Ç¨¢", color: T.red, label: "Urgent", pulse: true };
    if (scores.frustrated >= 3) return { sentiment: "frustrated", emoji: "‚‚Ç¨¢", color: T.red, label: "Frustrated" };
    if (scores.concerned >= 2) return { sentiment: "concerned", emoji: "‚‚Ç¨¢", color: T.orange, label: "Concerned" };
    if (scores.positive >= 2) return { sentiment: "positive", emoji: "‚‚Ç¨¢", color: T.green, label: "Positive" };
    
    return { sentiment: "neutral", emoji: "üüâ", color: T.textMuted, label: "Neutral" };
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div className="fade-up" style={{ display: "flex", alignItems: "center", gap: 0, borderRadius: 14, background: T.surface, border: `1px solid ${T.border}`, overflow: "hidden" }}>
        ...
        {[
          ...
          { label: "Avg Response", value: avgResponseHrs !== null ? `${avgResponseHrs}h` : "‚‚Ç¨‚Äù", color: avgResponseHrs !== null && avgResponseHrs > 24 ? T.red : T.green },
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 16px", marginLeft: "auto" }}>
          ...
          <span style={{ fontSize: 10, padding: "3px 6px", borderRadius: 4, background: `${T.blue}20`, color: T.blue }}>ü‚Äú∫ {tickets.filter(t => !t.channel || t.channel === "email").length}</span>
          <span style={{ fontSize: 10, padding: "3px 6px", borderRadius: 4, background: `${T.green}20`, color: T.green }}>ü‚Äôº {liveChats.filter(c => c.status === "active").length + tickets.filter(t => t.channel === "chat").length}</span>
          <span style={{ fontSize: 10, padding: "3px 6px", borderRadius: 4, background: "#25D36620", color: "#25D366" }}>ü‚Äú± {whatsappConversations.filter(c => c.status === "active").length + tickets.filter(t => t.channel === "whatsapp").length}</span>
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", gap: 8 }}>
          {[
            ...
            { id: "livechat", label: `ü‚Äôº Live Chat${liveChats.filter(c => c.status === "active").length > 0 ? ` (${liveChats.filter(c => c.status === "active").length})` : ""}` },
            { id: "whatsapp", label: `ü‚Äú± WhatsApp${whatsappConversations.filter(c => c.status === "active").length > 0 ? ` (${whatsappConversations.filter(c => c.status === "active").length})` : ""}` },
            { id: "analytics", label: "ü‚Äúà Analytics" },
            { id: "kb", label: "ü‚Äú¨ KB & Tools" },
            { id: "timetrack", label: `‚‚Äì‚ÄôÔ∏è Time${activeTimer ? " ‚‚Ç¨¢" : ""}` },
            { id: "auditlog", label: "ü‚Äúã Audit" },
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <select value={channelFilter} onChange={e => setChannelFilter(e.target.value)}
            ...
            <option value="email">ü‚Äú∫ Email</option>
            <option value="chat">ü‚Äôº Live Chat</option>
            <option value="whatsapp">ü‚Äú± WhatsApp</option>
            <option value="phone">ü‚Äúß Phone</option>
          </select>
          ...
          <select value={ticketPriorityFilter} onChange={e => setTicketPriorityFilter(e.target.value)}
            ...
            <option value="urgent">‚‚Ç¨¢ Urgent</option>
            <option value="high">‚‚Ç¨¢ High</option>
          ...
          <select value={tagFilter} onChange={e => setTagFilter(e.target.value)}
            ...
            {availableTags.map(t => <option key={t.id} value={t.id}>‚‚Ç¨¢ {t.label}</option>)}
          </select>
          <select value={assignmentFilter} onChange={e => setAssignmentFilter(e.target.value)}
            ...
            <option value="unassigned">‚‚Ç¨¢ Unassigned</option>
            {assignableAgents.filter(a => a.id !== "unassigned").map(a => <option key={a.id} value={a.id}>‚‚Ç¨¢ {a.name}</option>)}
          </select>
          {/* Custom Field Filter */}
          {customFields.length > 0 && (
            <select value={customFieldFilter.fieldId} onChange={e => setCustomFieldFilter({ fieldId: e.target.value, value: "" })}
              ...
              {customFields.map(f => <option key={f.id} value={f.id}>ü‚Äúã {f.name}</option>)}
            </select>
          )}
          ...
          <button type="button" onClick={() => setShowAutoAssignModal(true)}
            ...
            ü‚Äòõ {autoAssignRules.filter(r => r.enabled).length > 0 && <span style={{ fontSize: 9 }}>{autoAssignRules.filter(r => r.enabled).length}</span>}
          </button>
          ...
          <button type="button" onClick={() => setShowWorkflowModal(true)}
            ...
            ‚ö° {workflowTriggers.filter(w => w.enabled).length > 0 && <span style={{ fontSize: 9 }}>{workflowTriggers.filter(w => w.enabled).length}</span>}
          </button>
        </div>
        )}
      ...
      {supportSubTab === "analytics" ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                ...
                {analytics.volumeChange !== 0 && (
                  <span style={{ fontSize: 12, fontWeight: 600, color: analytics.volumeChange > 0 ? T.red : T.green }}>
                    {analytics.volumeChange > 0 ? "‚Üê" : "‚Ü‚Äò"} {Math.abs(analytics.volumeChange)}%
                  </span>
                )}
              </div>
            ...
            <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
              ...
              <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                <span style={{ fontSize: 32, fontWeight: 900, color: analytics.avgResolutionHrs && analytics.avgResolutionHrs > slaSettings.defaultHours ? T.red : T.green, fontFamily: "'Fraunces',serif" }}>
                  {analytics.avgResolutionHrs ? `${analytics.avgResolutionHrs}h` : "‚‚Ç¨‚Äù"}
                </span>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                ...
                {analytics.slaChange !== 0 && (
                  <span style={{ fontSize: 12, fontWeight: 600, color: analytics.slaChange > 0 ? T.green : T.red }}>
                    {analytics.slaChange > 0 ? "‚Üê" : "‚Ü‚Äò"} {Math.abs(analytics.slaChange)}%
                  </span>
                )}
              </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
            {/* Ticket Volume Chart */}
            <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 16 }}>ü‚Äúä Ticket Volume</div>
            ...
            {/* Category Breakdown */}
            <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 16 }}>ü‚Äúà By Category</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
            {/* Priority Breakdown */}
            <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 16 }}>‚‚Ç¨¢ By Priority</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {/* Status Breakdown */}
            <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 16 }}>ü‚Äúã By Status</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {/* Resolution Time Buckets */}
            <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 16 }}>‚‚Äì‚ÄôÔ∏è Resolution Time</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white }}>‚‚Ç¨¢ Agent Performance</div>
                    ...
                    {analytics.agentPerformance.slice(0, 10).map((agent, idx) => (
                      <tr key={agent.id} style={{ borderBottom: `1px solid ${T.border}20` }}>
                        <td style={{ padding: "12px", display: "flex", alignItems: "center", gap: 10 }}>
                          ...
                          <div>
                            ...
                            {idx === 0 && analytics.agentPerformance.length > 1 && (
                              <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: `${T.gold}20`, color: T.gold }}>‚‚Ç¨¢ Top Performer</span>
                            )}
                          </div>
                        </td>
                        ...
                        <td style={{ textAlign: "center", padding: "12px", color: agent.avgResolution && agent.avgResolution > slaSettings.defaultHours ? T.red : T.textSecondary }}>
                          {agent.avgResolution ? `${agent.avgResolution}h` : "‚‚Ç¨‚Äù"}
                        </td>
                        <td style={{ textAlign: "center", padding: "12px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
            {/* SLA Trend Chart */}
            <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 16 }}>ü‚Äúä SLA Compliance Trend</div>
            ...
            <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 16 }}>‚è∞Ô∏è Current Workload</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      ...
                      <div key={agent.id}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                          <span style={{ fontSize: 11, color: T.textSecondary, display: "flex", alignItems: "center", gap: 6 }}>
                            {agent.name}
                            {isOverloaded && <span style={{ fontSize: 9, padding: "1px 4px", borderRadius: 3, background: `${T.red}20`, color: T.red }}>‚ö°Ô∏è</span>}
                          </span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 8 }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>‚‚Ç¨¢ Customer Satisfaction</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
            {/* CSAT Trend */}
            <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 16 }}>ü‚Äúä CSAT Trend</div>
              <div style={{ height: 180 }}>
                <ResponsiveContainer width="100%" height="100%">
          ...
          <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: T.white }}>‚‚Ç¨¢ Agent CSAT Scores</div>
          ...
          {/* Recent Feedback */}
          <div style={{ padding: 20, background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 16 }}>ü‚Äôº Recent Feedback</div>
                ...
                {analytics.recentFeedback.slice(0, 5).map(feedback => (
                  <div key={feedback.id} style={{ padding: 14, background: T.surfaceAlt, borderRadius: 10, borderLeft: `3px solid ${feedback.rating >= 4 ? T.green : feedback.rating >= 3 ? T.orange : T.red}` }}>
                    ...
                    <div style={{ marginTop: 8, fontSize: 10, color: T.textMuted }}>
                      Agent: <span style={{ color: T.purple }}>{feedback.agentName || "‚‚Ç¨‚Äù"}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
            {[
              { label: "Active Chats", value: liveChats.filter(c => c.status === "active").length, color: T.green, icon: "ü‚Äôº" },
              { label: "In Queue", value: chatQueue.length, color: chatQueue.length > 0 ? T.orange : T.textMuted, icon: "" },
              { label: "Handled Today", value: liveChats.filter(c => c.status === "ended").length, color: T.teal, icon: "‚ú‚Äù" },
              { label: "Avg Wait", value: chatQueue.length > 0 ? `${Math.round(chatQueue.reduce((a, c) => a + (Date.now() - new Date(c.queuedAt).getTime()) / 1000, 0) / chatQueue.length / 60)}m` : "‚‚Ç¨‚Äù", color: T.textSecondary, icon: "‚‚Äì‚ÄôÔ∏è" },
              { label: "Avg Duration", value: liveChats.filter(c => c.duration).length > 0 ? `${Math.round(liveChats.filter(c => c.duration).reduce((a, c) => a + c.duration, 0) / liveChats.filter(c => c.duration).length / 60)}m` : "‚‚Ç¨‚Äù", color: T.textSecondary, icon: "ü‚Äúà" },
            ].map((stat, i) => (
          ...
          <div style={{ display: "grid", gridTemplateColumns: activeChatId ? "300px 1fr" : "1fr 1fr", gap: 16 }}>
            ...
            {activeChatId ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden" }}>
                    ...
                    <div style={{ padding: "12px 16px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{ fontSize: 12, color: T.textMuted, fontFamily: "'Fraunces',serif" }}>
                          ‚‚Ç¨¢ {durMins}:{durSecs.toString().padStart(2, "0")}
                        </div>
                    ...
                    <div style={{ padding: "8px 12px", borderTop: `1px solid ${T.border}`, background: T.surfaceAlt }}>
                      <button type="button" onClick={() => {
                        ...
                        ü‚Äôº Simulate Visitor Reply (Demo)
                      </button>
                    </div>
                  </div>
                );
              ...
              <div style={{ padding: 16, background: T.surface, borderRadius: 12, border: `1px solid ${T.border}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: T.white }}>‚‚Ç¨¢ Active Chats</span>
                ...
                {liveChats.filter(c => c.status === "active").length === 0 ? (
                  <div style={{ padding: 30, textAlign: "center", color: T.textMuted }}>
                    <div style={{ fontSize: 28, marginBottom: 8 }}>ü‚Äôº</div>
                      ...
                      return (
                        <div key={chat.id} onClick={() => setActiveChatId(chat.id)}
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            ...
                            <span style={{ fontSize: 10, color: T.green }}>Click to open ‚Ü‚Äô</span>
                          </div>
                        </div>
                      );
                    })}
                        ...
                        <div key={chat.id} style={{ padding: 10, background: T.surfaceAlt, borderRadius: 8, opacity: 0.7 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              ...
                              {chat.convertedToTicket && <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: `${T.teal}20`, color: T.teal }}>‚Ü‚Äô Ticket</span>}
                            </div>
                            <span style={{ fontSize: 10, color: T.textMuted }}>{chat.duration ? `${Math.floor(chat.duration / 60)}m ${chat.duration % 60}s` : "‚‚Ç¨‚Äù"}</span>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          {/* Embed Code Section */}
          <div style={{ padding: 16, background: T.surface, borderRadius: 12, border: `1px solid ${T.border}` }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 12 }}>ü‚Äî‚Ñ¢ Widget Embed Code</div>
  ...
  DXBChat.init({
    ...
    position: "${chatSettings.widgetPosition}"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#25D366", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <span style={{ fontSize: 18 }}>ü‚Äú±</span>
                </div>
                <div>
              ...
              <span style={{ fontSize: 10, padding: "4px 10px", borderRadius: 6, background: "#25D36620", color: "#25D366", fontWeight: 600 }}>‚ú‚Äù Connected</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
            {[
              { label: "Active", value: whatsappConversations.filter(c => c.status === "active").length, color: "#25D366", icon: "ü‚Äôº" },
              { label: "Unread", value: whatsappConversations.filter(c => !c.responded && c.status === "active").length, color: T.orange, icon: "ü‚Äù‚Äù" },
              { label: "Today", value: whatsappConversations.filter(c => new Date(c.createdAt) > new Date(Date.now() - 86400000)).length, color: T.teal, icon: "ü‚Äúà" },
              { label: "Converted", value: whatsappConversations.filter(c => c.convertedToTicket).length, color: T.purple, icon: "‚‚Ç¨¢" },
              { label: "Avg Response", value: "~5m", color: T.textSecondary, icon: "‚‚Äì‚ÄôÔ∏è" },
            ].map((stat, i) => (
              <div key={i} style={{ padding: 16, background: T.surface, borderRadius: 10, border: `1px solid ${T.border}`, textAlign: "center" }}>
                <div style={{ fontSize: 20, marginBottom: 4 }}>{stat.icon}</div>
                <div style={{ fontSize: 24, fontWeight: 900, color: stat.color, fontFamily: "'Fraunces',serif" }}>{stat.value}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: activeWhatsappId ? "300px 1fr" : "1fr", gap: 16 }}>
            ...
            <div style={{ padding: 16, background: T.surface, borderRadius: 12, border: `1px solid ${T.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: T.white }}>ü‚Äú± Conversations</span>
              ...
              {whatsappConversations.length === 0 ? (
                <div style={{ padding: 30, textAlign: "center", color: T.textMuted }}>
                  <div style={{ fontSize: 28, marginBottom: 8 }}>ü‚Äú±</div>
                    ...
                    return (
                      <div key={conv.id} onClick={() => setActiveWhatsappId(conv.id)}
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          ...
                          {windowRemaining > 0 ? (
                            <span style={{ fontSize: 9, color: windowRemaining < 4 ? T.orange : T.textMuted }}>‚‚Äì‚ÄôÔ∏è {Math.round(windowRemaining)}h window</span>
                          ) : (
                            <span style={{ fontSize: 9, color: T.red }}>‚ö°Ô∏è Window expired</span>
                          )}
                        </div>
                      </div>
                    );
                ...
                <div style={{ display: "flex", flexDirection: "column", background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden" }}>
                    ...
                    {(conv.messages || []).map((msg, i) => (
                      <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: msg.from === "agent" ? "flex-end" : "flex-start" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 4 }}>
                          ...
                          {msg.from === "agent" && <span style={{ fontSize: 9, color: "#53BDEB" }}>‚ú‚Äù‚ú‚Äù</span>}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ padding: "8px 12px", borderTop: `1px solid ${T.border}`, background: T.surfaceAlt }}>
                    <button type="button" onClick={() => {
                      ...
                      style={{ width: "100%", padding: "6px 12px", borderRadius: 5, border: `1px dashed ${T.border}`, background: "transparent", color: T.textMuted, fontSize: 10, cursor: "pointer" }}>
                      ü‚Äú± Simulate Customer Reply (Demo)
                    </button>
                  </div>
                </div>
              );
          ...
          <div style={{ padding: 16, background: T.surface, borderRadius: 12, border: `1px solid ${T.border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: T.white, marginBottom: 8 }}>ü‚Äú± WhatsApp Business API</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {(() => {
            const kbCategories = [
              ...
              { id: "account", label: "Account Management", icon: "‚‚Ç¨¢" },
            ];
            
            const qrCategories = [
              { id: "general", label: "General", icon: "ü‚Äôº" },
              { id: "technical", label: "Technical", icon: "‚‚Ç¨¢" },
              { id: "billing", label: "Billing", icon: "‚‚Ç¨¢" },
            ];
                  ...
                  {viewingArticle ? (
                    <div style={{ padding: 20, background: T.surfaceAlt, borderRadius: 10 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                        ...
                        <span style={{ fontSize: 10, color: T.textMuted }}>‚‚Ç¨¢ {viewingArticle.views || 0} views</span>
                        <span style={{ fontSize: 10, color: T.textMuted }}>üéå {viewingArticle.helpful || 0} helpful</span>
                      </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {articlesByCategory.length === 0 ? (
                        <div style={{ padding: 30, textAlign: "center", color: T.textMuted }}>
                          <div style={{ fontSize: 24, marginBottom: 8 }}>ü‚Äú¨</div>
                        ...
                        articlesByCategory.map(cat => (
                          <div key={cat.id} style={{ background: T.surfaceAlt, borderRadius: 10, overflow: "hidden" }}>
                            <button type="button" onClick={() => setExpandedKbCategory(expandedKbCategory === cat.id ? null : cat.id)}
                              ...
                              <span style={{ color: T.textMuted, fontSize: 12 }}>{expandedKbCategory === cat.id ? "‚‚Äîº" : "‚‚Äì∂"}</span>
                            </button>
                                ...
                                {cat.articles.map(article => (
                                  <div key={article.id} onClick={() => setViewingArticle(article)}
                                    ...
                                    <div style={{ display: "flex", gap: 12, fontSize: 10, color: T.textMuted }}>
                                      <span>‚‚Ç¨¢ {article.views || 0}</span>
                                      <span>üéå {article.helpful || 0}</span>
                                    </div>
                                  </div>
                                ))}
                              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div style={{ fontSize: 11, color: T.textMuted }}>
              üÜç Start timer from any ticket drawer, or add manual entries here
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 20 }}>
            <div style={{ background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, padding: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: T.white, marginBottom: 16 }}>‚‚Ç¨¢ Agent Breakdown</div>
            ...
            <div style={{ background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden" }}>
              <div style={{ padding: 16, borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: T.white }}>‚‚Ç¨¢ Recent Time Entries</div>
              ...
              <div style={{ maxHeight: 350, overflowY: "auto" }}>
                    ...
                    <div key={entry.id} style={{ padding: "12px 16px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 10, color: T.textMuted, marginTop: 2 }}>
                          {entry.agentName} ‚‚Ç¨¢ {entry.notes} ‚‚Ç¨¢ {timeAgo(entry.createdAt)}
                        </div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                {timeEntries.length === 0 && (
                  <div style={{ padding: 40, textAlign: "center", color: T.textMuted }}>
                    <div style={{ fontSize: 24, marginBottom: 8 }}>‚‚Äì‚ÄôÔ∏è</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <div style={{ padding: 16, borderBottom: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: T.white }}>ü‚Äúã Ticket Audit Trail</div>
            </div>
            <div style={{ maxHeight: 500, overflowY: "auto" }}>
                  ...
                  <div key={log.id} style={{ padding: "14px 16px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "flex-start", gap: 12 }}
                    ...
                    <div style={{ width: 32, height: 32, borderRadius: 8, background: `${getAuditActionColor(log.action)}20`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <span style={{ fontSize: 12 }}>
                        {log.action === "created" ? "‚‚Ç¨¢" :
                         log.action === "status_change" ? "ü‚ÄîÑ" :
                         log.action === "assigned" ? "‚‚Ç¨¢" :
                         log.action === "reply_sent" ? "ü‚Äôº" :
                         log.action === "note_added" ? "ü‚Äúé" :
                         log.action === "tag_added" ? "‚‚Ç¨¢" :
                         log.action === "tag_removed" ? "‚‚Ç¨¢" :
                         log.action === "time_logged" ? "‚‚Äì‚ÄôÔ∏è" :
                         log.action === "escalated" ? "ü‚Äù•" :
                         log.action === "merged" ? "ü‚Äî‚Ñ¢" : "ü‚Äúã"}
                      </span>
                    </div>
                      ...
                      {log.details && (
                        <div style={{ fontSize: 10, color: T.textMuted, marginTop: 4 }}>
                          {log.action === "status_change" && log.details.from && log.details.to && (
                            <span>{log.details.from} ‚Ü‚Äô {log.details.to}</span>
                          )}
              ...
              {ticketAuditLogs.length === 0 && (
                <div style={{ padding: 60, textAlign: "center", color: T.textMuted }}>
                  <div style={{ fontSize: 32, marginBottom: 12 }}>ü‚Äúã</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            ...
            <div style={{ background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden" }}>
              <div style={{ padding: 16, borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: T.white }}>ü‚Äî‚Ñ¢ Webhooks</div>
              ...
              <div style={{ maxHeight: 280, overflowY: "auto" }}>
                {webhooks.length === 0 ? (
                  <div style={{ padding: 40, textAlign: "center", color: T.textMuted }}>
                    <div style={{ fontSize: 24, marginBottom: 8 }}>ü‚Äî‚Ñ¢</div>
            ...
            <div style={{ background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden" }}>
              <div style={{ padding: 16, borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: T.white }}>ü‚Äîâ Agent Permissions</div>
              ...
              <div style={{ maxHeight: 280, overflowY: "auto" }}>
                {agentPermissions.length === 0 ? (
                  <div style={{ padding: 40, textAlign: "center", color: T.textMuted }}>
                    <div style={{ fontSize: 24, marginBottom: 8 }}>ü‚Äîâ</div>
          ...
          <div style={{ background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <div style={{ padding: 16, borderBottom: `1px solid ${T.border}` }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: T.white }}>ü‚Äúú Webhook Logs</div>
                ...
                webhookLogs.slice(0, 20).map(log => (
                  <div key={log.id} style={{ padding: "10px 16px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 10, color: T.textMuted }}>{log.event} ‚‚Ç¨¢ {log.statusCode}</div>
                    </div>
          ...
          {/* Role Descriptions */}
          <div style={{ background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, padding: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: T.white, marginBottom: 12 }}>ü‚Äúõ Role Permissions</div>
                ...
                <div key={ticket.id} onClick={() => { setTicketDrawer(ticket); setTicketSummary(null); setSuggestedReplies([]); setSimilarTickets([]); setShowSimilarTickets(false); }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                      ...
                      {ticket.channel && ticket.channel !== "email" && <span style={{ fontSize: 8, padding: "2px 6px", borderRadius: 4, background: ticket.channel === "chat" ? `${T.green}20` : ticket.channel === "whatsapp" ? "#25D36620" : `${T.purple}20`, color: ticket.channel === "chat" ? T.green : ticket.channel === "whatsapp" ? "#25D366" : T.purple, fontWeight: 600 }}>{ticket.channel === "chat" ? "ü‚Äôº CHAT" : ticket.channel === "whatsapp" ? "ü‚Äú± WA" : "ü‚Äúß CALL"}</span>}
                      {slaInfo.status === "breached" && <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: `${T.red}20`, color: T.red, fontWeight: 600 }}>‚‚Äì‚Äò SLA {slaInfo.percent}%</span>}
                      {slaInfo.status === "warning" && <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: `${T.orange}20`, color: T.orange, fontWeight: 600 }}>‚ö°Ô∏è {slaInfo.percent}%</span>}
                      {ticket.autoAssignedBy && <span style={{ fontSize: 8, padding: "2px 6px", borderRadius: 4, background: `${T.green}20`, color: T.green, fontWeight: 600 }}>ü‚Äòõ Auto</span>}
                      {ticket.autoEscalated && <span style={{ fontSize: 8, padding: "2px 6px", borderRadius: 4, background: `${T.red}20`, color: T.red, fontWeight: 600 }}>‚ö° Escalated</span>}
                      ...
                      {(ticket.linkedTickets || []).length > 0 && !ticket.mergedInto && <span style={{ fontSize: 8, padding: "2px 6px", borderRadius: 4, background: `${T.teal}20`, color: T.teal, fontWeight: 600 }}>ü‚Äî‚Ñ¢ {ticket.linkedTickets.length}</span>}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, color: T.textMuted }}>
                      ...
                      {ticket.assignedTo && <><span>¬∑</span><span style={{ color: T.purple }}>‚‚Ç¨¢ {ticket.assignedToName || "Assigned"}</span></>}
                      {/* SLA Progress Bar */}
            ...
            {viewingAdmins.length > 0 && (
              <div style={{ padding: "10px 24px", background: `${T.purple}15`, borderBottom: `1px solid ${T.purple}30`, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 11, color: T.purple, fontWeight: 500 }}>
                  ‚‚Ç¨¢ {viewingAdmins.map(a => a.name).join(", ")} {viewingAdmins.length === 1 ? "is" : "are"} also viewing this ticket
                </span>
              </div>
            )}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
                    ...
                    {isSlaBreached(ticketDrawer) && <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: `${T.red}20`, color: T.red, fontWeight: 600 }}>‚‚Äì‚Äò SLA Breached</span>}
                    {/* Sentiment Badge */}
                    {(() => {
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <button type="button" onClick={() => setTicketDrawer(null)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 24, lineHeight: 1 }}>√‚Äî</button>
                </div>
              </div>
                ...
                {(ticketDrawer.tags || []).map(tagId => {
                  ...
                  return tag ? (
                    <span key={tagId} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 10, padding: "3px 8px", borderRadius: 4, background: `${tag.color}20`, color: tag.color, fontWeight: 600 }}>
                      {tag.label}
                      <button type="button" onClick={() => removeTag(ticketDrawer.id, tagId)} style={{ background: "none", border: "none", color: tag.color, cursor: "pointer", fontSize: 12, lineHeight: 1, padding: 0 }}>√‚Äî</button>
                    </span>
                  ) : null;
                })}
              ...
              {(ticketDrawer.linkedTickets || []).length > 0 && (
                <div style={{ marginTop: 12, padding: "10px 12px", background: `${T.teal}08`, borderRadius: 8, border: `1px solid ${T.teal}20` }}>
                  <div style={{ fontSize: 10, fontWeight: 600, color: T.teal, marginBottom: 8, display: "flex", alignItems: "center", gap: 4 }}>
                    ü‚Äî‚Ñ¢ Linked Tickets ({ticketDrawer.linkedTickets.length})
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <div style={{ marginTop: 12, padding: "10px 12px", background: `${T.gold}08`, borderRadius: 8, border: `1px solid ${T.gold}20` }}>
                ...
                {activeTimer && activeTimer.ticketId === ticketDrawer.id ? (
                  ...
                  <button type="button" onClick={() => startTimer(ticketDrawer.id)} disabled={activeTimer !== null}
                    ...
                    ‚‚Äì∂Ô∏è {activeTimer ? "Timer running on another ticket" : "Start Timer"}
                  </button>
                )}
                    ...
                    {timeEntries.filter(e => e.ticketId === ticketDrawer.id).slice(0, 5).map(entry => (
                      <div key={entry.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 10px", background: T.surface, borderRadius: 6 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          {entry.notes && <span style={{ fontSize: 9, color: T.textMuted }}>‚‚Ç¨¢ {entry.notes.slice(0, 20)}</span>}
                        </div>
            ...
            {(ticketDrawer.status === "resolved" || ticketDrawer.status === "closed") && (() => {
              ...
              return (
                <div style={{ padding: "12px 24px", background: `${T.gold}08`, borderBottom: `1px solid ${T.gold}20` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontSize: 11, fontWeight: 600, color: T.gold }}>‚‚Ç¨¢ Customer Satisfaction</span>
                      {ticketCsat ? (
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {showAiPanel && (
              <div style={{ padding: "16px 24px", background: `${T.teal}08`, borderBottom: `1px solid ${T.teal}20` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: T.teal }}>ü‚Äòõ AI Insights</span>
                      ...
                      <div style={{ flex: 1, padding: 12, background: T.surface, borderRadius: 8, border: `1px solid ${needsChange ? T.gold : T.border}30` }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          <div>
                            ...
                            <div style={{ fontSize: 10, color: T.textMuted }}>
                              {aiPriority.reasons.slice(0, 2).join(" ‚‚Ç¨¢ ")}
                            </div>
                          </div>
                ...
                {ticketSummary ? (
                  <div style={{ padding: 14, background: T.surface, borderRadius: 8, border: `1px solid ${T.border}` }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                          {ticketSummary.keyPoints.map((point, i) => (
                            <div key={i} style={{ fontSize: 11, color: T.textSecondary, display: "flex", alignItems: "flex-start", gap: 6 }}>
                              <span style={{ color: T.teal }}>‚‚Ç¨¢</span> {point}
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 10, borderTop: `1px solid ${T.border}` }}>
                      ...
                      <div style={{ fontSize: 10, padding: "3px 8px", borderRadius: 4, background: `${T.teal}20`, color: T.teal, fontWeight: 600 }}>
                        üÜç {ticketSummary.aiRecommendation}
                      </div>
                    </div>
                  </div>
                ) : !summaryLoading ? (
                  ...
                  <div style={{ padding: 16, background: T.surface, borderRadius: 8, textAlign: "center" }}>
                    <div style={{ fontSize: 11, color: T.teal }}>ü‚ÄîÑ Analyzing ticket content...</div>
                  </div>
                )}
                ...
                <div style={{ marginTop: 12 }}>
                  <button type="button" onClick={() => { if (!showSimilarTickets) updateSimilarTickets(ticketDrawer); setShowSimilarTickets(!showSimilarTickets); }}
                    ...
                    <span>ü‚Äî‚Ñ¢ Similar Resolved Tickets {similarTickets.length > 0 && `(${similarTickets.length})`}</span>
                    <span style={{ fontSize: 10, color: T.textMuted }}>{showSimilarTickets ? "" : "‚‚Äîº"}</span>
                  </button>
                            ...
                            <div key={sim.id} style={{ padding: 12, background: T.surface, borderRadius: 8, border: `1px solid ${T.border}`, cursor: "pointer" }}
                              ...
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                                  <span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: `${T.green}20`, color: T.green }}>‚ú‚Äù Resolved</span>
                              ...
                              {sim.solution && (
                                <div style={{ padding: 8, background: `${T.green}08`, borderRadius: 6, borderLeft: `2px solid ${T.green}` }}>
                                  <div style={{ fontSize: 9, color: T.textMuted, marginBottom: 4 }}>üÜç Resolution:</div>
                                  <div style={{ fontSize: 11, color: T.textSecondary, lineHeight: 1.4 }}>{sim.solution}</div>
                                </div>
                              )}
                              <div style={{ marginTop: 8, display: "flex", justifyContent: "flex-end" }}>
                                <span style={{ fontSize: 9, color: T.teal }}>Click to view full ticket ‚Ü‚Äô</span>
                              </div>
                            </div>
                          ))}
                        </div>
                          ...
                          <div style={{ marginTop: 8, padding: 8, background: msg.from === "admin" ? "rgba(0,0,0,0.15)" : T.surface, borderRadius: 8 }}>
                            {msg.attachment.type?.startsWith("image/") ? (
                              ...
                              <a href={msg.attachment.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, color: msg.from === "admin" ? T.bg : T.teal, textDecoration: "none" }}>
                                <span style={{ fontSize: 20 }}>ü‚ÄúÑ</span>
                                <div>
              ...
              {(ticketDrawer.internalNotes?.length > 0 || true) && (
                <div style={{ padding: "16px 24px", background: `${T.orange}08`, borderTop: `1px dashed ${T.orange}30` }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: T.orange, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
                    ü‚Äîé Internal Notes <span style={{ fontWeight: 400, color: T.textMuted }}>(hidden from customer)</span>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                          ...
                          <div style={{ marginTop: 8, padding: 8, background: T.surfaceAlt, borderRadius: 6 }}>
                            {note.attachment.type?.startsWith("image/") ? (
                              ...
                              <a href={note.attachment.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 8, color: T.teal, textDecoration: "none" }}>
                                <span style={{ fontSize: 18 }}>ü‚ÄúÑ</span>
                                <div>
              ...
              <div style={{ padding: "16px 24px", borderTop: `1px solid ${T.border}`, background: T.surfaceAlt }}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
                  ...
                  <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
                    ...
                    {uploading ? " Uploading..." : "ü‚ÄúÑ Attach File"}
                  </button>
                </div>
                ...
                {replyMode === "reply" ? (
                    ...
                    <div style={{ marginBottom: 10 }}>
                      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 8 }}>
                        <button type="button" onClick={() => setShowTemplates(!showTemplates)}
                          ...
                          {showTemplates ? "Hide Templates" : "‚‚Ç¨¢ Quick Templates"}
                        </button>
                        <button type="button" onClick={() => { if (suggestedReplies.length === 0) generateSmartReplies(ticketDrawer); }}
                          ...
                          {repliesLoading ? " Generating..." : suggestedReplies.length > 0 ? `üÜç ${suggestedReplies.length} AI Suggestions` : "üÜç Generate Smart Replies"}
                        </button>
                        {quickResponses.length > 0 && (
                          <span style={{ fontSize: 10, color: T.textMuted }}>
                            ‚ö° {quickResponses.length} quick responses
                          </span>
                        )}
                      </div>
                      ...
                      {suggestedReplies.length > 0 && (
                        <div style={{ marginBottom: 12, padding: 12, background: `${T.purple}08`, borderRadius: 10, border: `1px solid ${T.purple}20` }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                            <span style={{ fontSize: 11, fontWeight: 600, color: T.purple }}>üÜç AI-Suggested Replies</span>
                            ...
                            {suggestedReplies.map(reply => (
                              <div key={reply.id} style={{ padding: 10, background: T.surface, borderRadius: 8, border: `1px solid ${T.border}`, cursor: "pointer", transition: "all 0.15s" }}
                                ...
                                <div style={{ marginTop: 6, display: "flex", justifyContent: "flex-end" }}>
                                  <span style={{ fontSize: 9, color: T.purple }}>Click to use ‚Ü‚Äô</span>
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      {showTemplates && (
                        <div style={{ marginTop: 8 }}>
                          ...
                          {quickResponses.length > 0 && (
                            <>
                              <div style={{ fontSize: 9, color: T.textMuted, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>‚ö° Quick Responses</div>
                  ...
                  <>
                    <div style={{ marginBottom: 10, display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                      ...
                      <span style={{ fontSize: 10, color: T.textMuted, marginLeft: 8 }}>üÜç Type @ to mention teammates</span>
                    </div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowMergeModal(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 480, maxHeight: "80vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>‚‚Ç¨¢ Merge Ticket</h3>
              <button type="button" onClick={() => setShowMergeModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            
            <div style={{ padding: 12, background: `${T.orange}15`, borderRadius: 8, marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: T.orange, fontWeight: 600, marginBottom: 4 }}>‚ö°Ô∏è Warning</div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ padding: 12, background: T.surfaceAlt, borderRadius: 8, border: `1px solid ${T.border}` }}>
                ...
                <div style={{ fontSize: 11, color: T.textMuted }}>{ticketDrawer.userEmail} ‚‚Ç¨¢ {(ticketDrawer.messages || []).length} messages</div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              {mergeTargetId && (
                <div style={{ marginTop: 10, padding: 10, background: `${T.green}10`, borderRadius: 6, border: `1px solid ${T.green}30` }}>
                  <div style={{ fontSize: 11, color: T.green }}>‚ú‚Äù Messages and notes will be combined into this ticket</div>
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button type="button" onClick={mergeTickets} disabled={!mergeTargetId || merging}
                ...
                {merging ? "Merging..." : "‚‚Ç¨¢ Merge Tickets"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showLinkModal && ticketDrawer && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowLinkModal(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 480, maxHeight: "80vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>ü‚Äî‚Ñ¢ Link Related Ticket</h3>
              <button type="button" onClick={() => setShowLinkModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button type="button" onClick={linkTicket} disabled={!linkTargetId || linking}
                ...
                {linking ? "Linking..." : "ü‚Äî‚Ñ¢ Link Tickets"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowFieldsModal(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 560, maxHeight: "85vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <button type="button" onClick={() => setShowFieldsModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            ...
            <div style={{ padding: 16, background: T.surfaceAlt, borderRadius: 10, marginBottom: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: T.white, marginBottom: 12 }}>
                {editingField ? "‚ú‚Ä¶Ô∏è Edit Field" : "ü‚Äúå Add New Field"}
              </div>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: T.white, marginBottom: 12 }}>
                ü‚Äúã Existing Fields ({customFields.length})
              </div>
              {customFields.length === 0 ? (
                    ...
                    <div key={field.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: T.surfaceAlt, borderRadius: 8, border: `1px solid ${T.border}` }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <span style={{ fontSize: 18 }}>
                          {field.type === "dropdown" ? "ü‚Äúã" : field.type === "date" ? "ü‚Äú‚Ç¨" : field.type === "number" ? "ü‚Äî‚Äú" : field.type === "checkbox" ? "‚ö†Ô∏è" : "‚‚Ç¨¢"}
                        </span>
                        <div>
                          ...
                          <div style={{ fontSize: 10, color: T.textMuted }}>
                            ...
                            {field.type === "dropdown" && field.options?.length > 0 && ` ‚‚Ç¨¢ ${field.options.length} options`}
                          </div>
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
      ...
      {showAutoAssignModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowAutoAssignModal(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 600, maxHeight: "85vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>ü‚Äòõ Auto-Assign Rules</h3>
              <button type="button" onClick={() => setShowAutoAssignModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            
            <div style={{ padding: 12, background: `${T.green}15`, borderRadius: 8, marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: T.textSecondary, lineHeight: 1.5 }}>
                Create rules to automatically assign tickets based on category, priority, user tier, or keywords. Rules are applied in order ‚‚Ç¨‚Äù first match wins.
              </div>
            </div>
            ...
            <div style={{ padding: 16, background: T.surfaceAlt, borderRadius: 10, marginBottom: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: T.white, marginBottom: 12 }}>
                {editingRule ? "‚ú‚Ä¶Ô∏è Edit Rule" : "ü‚Äúå Add New Rule"}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div style={{ gridColumn: "span 2" }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: T.white, marginBottom: 12 }}>
                ü‚Äúã Active Rules ({autoAssignRules.length})
              </div>
                    ...
                    <div key={rule.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", background: T.surfaceAlt, borderRadius: 8, border: `1px solid ${rule.enabled ? T.green : T.border}40`, opacity: rule.enabled ? 1 : 0.6 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 10, color: T.textMuted }}>
                            If <span style={{ color: T.cyan }}>{rule.condition}</span> = <span style={{ color: T.gold }}>{rule.conditionValue}</span> ‚Ü‚Äô assign to <span style={{ color: T.purple }}>{rule.assignToName}</span>
                          </div>
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
            ...
            <div style={{ marginTop: 20, padding: 16, background: `${T.gold}10`, borderRadius: 10, border: `1px solid ${T.gold}30` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: T.gold }}>üÜç Run Auto-Assign on Unassigned Tickets</div>
      ...
      {showSlaModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowSlaModal(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 500, maxHeight: "85vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>‚‚Äì‚ÄôÔ∏è SLA Settings</h3>
              <button type="button" onClick={() => setShowSlaModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ padding: 16, background: `${T.blue}10`, borderRadius: 10, border: `1px solid ${T.blue}30` }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: T.blue, marginBottom: 8 }}>ü‚Äúà SLA Timeline Preview</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      {showWorkflowModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowWorkflowModal(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 650, maxHeight: "90vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>‚ö° Workflow Triggers</h3>
              <button type="button" onClick={() => setShowWorkflowModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            
            <div style={{ padding: 12, background: `${T.purple}15`, borderRadius: 8, marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: T.textSecondary, lineHeight: 1.5 }}>
                Create automated workflows that trigger actions when certain conditions are met. Example: When status changes to "resolved" ‚Ü‚Äô add "resolved" tag.
              </div>
            </div>
            ...
            <div style={{ padding: 16, background: T.surfaceAlt, borderRadius: 10, marginBottom: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: T.white, marginBottom: 12 }}>
                {editingWorkflow ? "‚ú‚Ä¶Ô∏è Edit Workflow" : "ü‚Äúå Create Workflow"}
              </div>
              
              {/* Workflow Name */}
              <div style={{ marginBottom: 12 }}>
                <label style={{ fontSize: 10, color: T.textMuted, marginBottom: 4, display: "block" }}>Workflow Name</label>
                <input value={newWorkflowForm.name} onChange={e => setNewWorkflowForm(prev => ({ ...prev, name: e.target.value }))} placeholder="e.g. Resolved ‚Ü‚Äô Add Tag..."
                  style={{ width: "100%", padding: "10px 12px", borderRadius: 6, border: `1px solid ${T.border}`, background: T.bg, color: T.white, fontSize: 13, fontFamily: "'Outfit',sans-serif", boxSizing: "border-box" }} />
              </div>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: T.white, marginBottom: 12 }}>
                ü‚Äúã Active Workflows ({workflowTriggers.length})
              </div>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {(wf.actions || []).map((action, aIdx) => (
                          <span key={aIdx} style={{ fontSize: 10, padding: "4px 8px", borderRadius: 4, background: `${T.purple}20`, color: T.purple }}>
                            {actionOptions.find(a => a.id === action.type)?.label || action.type}: {action.value || "‚‚Ç¨‚Äù"}
                          </span>
                        ))}
                      </div>
                    </div>
      ...
      {showCsatModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9100, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowCsatModal(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 420 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>‚‚Ç¨¢ Add CSAT Rating</h3>
              <button type="button" onClick={() => setShowCsatModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
          ...
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 600, maxHeight: "90vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>
                {editingArticle ? "‚ú‚Ä¶Ô∏è Edit Article" : "ü‚Äú¨ New Article"}
              </h3>
              <button type="button" onClick={() => setShowKbModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              <div>
                ...
                <select value={articleForm.category} onChange={e => setArticleForm(prev => ({ ...prev, category: e.target.value }))}
                  style={{ width: "100%", padding: "12px 14px", borderRadius: 8, border: `1px solid ${T.border}`, background: T.bg, color: T.white, fontSize: 13, fontFamily: "'Outfit',sans-serif" }}>
                  <option value="getting-started">ü‚Äùá Getting Started</option>
                  <option value="billing">‚‚Ç¨¢ Billing & Payments</option>
                  <option value="technical">‚‚Ç¨¢ Technical Issues</option>
                  <option value="features"> Features & How-To</option>
                  <option value="account">‚‚Ç¨¢ Account Management</option>
                </select>
              </div>
              <div>
      ...
      {showQuickResponseModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9100, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowQuickResponseModal(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 550, maxHeight: "90vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>
                {editingQuickResponse ? "‚ú‚Ä¶Ô∏è Edit Quick Response" : "‚ö° New Quick Response"}
              </h3>
              <button type="button" onClick={() => setShowQuickResponseModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <select value={quickResponseForm.category} onChange={e => setQuickResponseForm(prev => ({ ...prev, category: e.target.value }))}
                style={{ width: "100%", padding: "12px 14px", borderRadius: 8, border: `1px solid ${T.border}`, background: T.bg, color: T.white, fontSize: 13, fontFamily: "'Outfit',sans-serif" }}>
                <option value="general">ü‚Äôº General</option>
                <option value="technical">‚‚Ç¨¢ Technical</option>
                <option value="billing">‚‚Ç¨¢ Billing</option>
              </select>
            </div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 9100, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowChatSettings(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 500, maxHeight: "90vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <button type="button" onClick={() => setShowChatSettings(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 9100, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowWidgetPreview(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.gold}30`, padding: 24, width: "100%", maxWidth: 400 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <button type="button" onClick={() => setShowWidgetPreview(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            ...
            <div style={{ background: "#f5f5f5", borderRadius: 12, padding: 20, display: "flex", justifyContent: chatSettings.widgetPosition === "right" ? "flex-end" : "flex-start" }}>
              ...
              <div style={{ width: 320 }}>
                ...
                <div style={{ background: chatSettings.widgetColor, borderRadius: "12px 12px 0 0", padding: 16, color: T.white }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 40, height: 40, borderRadius: "50%", background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>ü‚Äôº</div>
                    <div>
                      ...
                      <div style={{ fontSize: 11, opacity: 0.9, display: "flex", alignItems: "center", gap: 4 }}>
                        ...
                        {agentOnline ? "Online ‚‚Ç¨¢ ~2 min" : "Offline"}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ background: "#fff", padding: 16, minHeight: 200 }}>
                  ...
                  {!agentOnline && (
                    <div style={{ background: "#FEF3C7", borderRadius: 8, padding: 10, fontSize: 11, color: "#92400E" }}>
                      ‚ö°Ô∏è {chatSettings.offlineMessage}
                    </div>
                  )}
                </div>
      ...
      {showWhatsappTemplates && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9100, background: "rgba(4,9,15,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowWhatsappTemplates(false)}>
          <div style={{ background: T.surface, borderRadius: 16, border: `1px solid #25D36630`, padding: 24, width: "100%", maxWidth: 550, maxHeight: "90vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 24 }}>ü‚Äú±</span>
                <div>
                  <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: T.white, fontFamily: "'Fraunces',serif" }}>
              ...
              <button type="button" onClick={() => setShowWhatsappTemplates(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            
            <div style={{ padding: 12, background: `${T.teal}10`, borderRadius: 8, marginBottom: 16, display: "flex", alignItems: "flex-start", gap: 10 }}>
              <span style={{ fontSize: 16 }}>üÜç</span>
                ...
                return (
                  <div key={template.id} 
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      ...
                      <span style={{ fontSize: 9, padding: "3px 8px", borderRadius: 4, background: "#25D36620", color: "#25D366" }}>‚ú‚Äù Approved</span>
                    </div>
                    ...
                    {activeConv && (
                      <div style={{ marginTop: 8, display: "flex", justifyContent: "flex-end" }}>
                        <span style={{ fontSize: 10, color: "#25D366" }}>Click to send ‚Ü‚Äô</span>
                      </div>
                    )}
                  </div>
                );
            ...
            <div style={{ marginTop: 20, padding: 12, background: T.surfaceAlt, borderRadius: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: T.textMuted, marginBottom: 6 }}>‚‚Ç¨¢ Create Custom Template</div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }}>
          <div style={{ width: 440, background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: T.white }}>‚‚Äì‚ÄôÔ∏è Add Manual Time Entry</div>
              <button type="button" onClick={() => setShowTimeEntryModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
            ...
            <div style={{ padding: 20 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                  ...
                  <span style={{ fontSize: 12, color: T.textSecondary }}>üèÜ Billable time</span>
                </label>
              </div>
      ...
      {showWebhookModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }}>
          <div style={{ width: 500, background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: T.white }}>ü‚Äî‚Ñ¢ {editingWebhook ? "Edit Webhook" : "Add Webhook"}</div>
              <button type="button" onClick={() => setShowWebhookModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
      ...
      {showExportModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }}>
          <div style={{ width: 460, background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: T.white }}>ü‚Äú‚Äò Export Tickets</div>
              <button type="button" onClick={() => setShowExportModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                    ...
                    <span style={{ fontSize: 12, color: T.textSecondary }}>ü‚Äôº Messages</span>
                  </label>
                  <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                    ...
                    <span style={{ fontSize: 12, color: T.textSecondary }}>ü‚Äúé Internal Notes</span>
                  </label>
                  <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                    ...
                    <span style={{ fontSize: 12, color: T.textSecondary }}>‚‚Äì‚ÄôÔ∏è Time Entries</span>
                  </label>
                </div>
              </div>
      ...
      {showPermissionsModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10000 }}>
          <div style={{ width: 420, background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, overflow: "hidden" }}>
            <div style={{ padding: "16px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: T.white }}>ü‚Äîâ {editingPermission ? "Edit Permission" : "Add Permission"}</div>
              <button type="button" onClick={() => setShowPermissionsModal(false)} style={{ background: "none", border: "none", color: T.textMuted, cursor: "pointer", fontSize: 20 }}>√‚Äî</button>
            </div>
...
/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ NOTIFICATIONS TAB COMPONENT ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */
/* ========================================
   TAB 12: NOTIFICATIONS ‚‚Ç¨‚Äù PRO LEVEL
