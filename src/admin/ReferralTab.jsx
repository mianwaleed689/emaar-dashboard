...
const ReferralTab = ({ db, T, notify, users, adminUser }) => {
  ...
  const grantReward = async (referral) => {
    ...
    try {
      ...
      await addDoc(collection(db, "adminAlerts"), {
        message: `üéÅ Referral reward granted: ${referral.referrerEmail} gets 1 month free`,
        type: "referral_reward", severity: "info", read: false,
        createdAt: new Date().toISOString(), source: "admin/referral",
      });
      notify("‚ú‚Ä¶ 1 month free granted to " + (referral.referrerEmail || "referrer"), "success");
    } catch(e) {
      notify("‚ùå Failed: " + e.message, "error");
  ...
  const createReferral = async (referrerUser, convertedEmail) => {
    try {
      ...
      notify("‚ú‚Ä¶ Referral conversion recorded", "success");
    } catch(e) {
      notify("‚ùå " + e.message, "error");
    }
  };

  // Leaderboard ‚‚Ç¨‚Äù group by referrer
    ...
    <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          <p style={{ color: T.textMuted, fontSize: 13 }}>Users share their link ‚Ü‚Äô friend signs up ‚Ü‚Äô converts to paid ‚Ü‚Äô referrer gets 1 month free</p>
        </div>
        <div style={{ fontSize: 9, padding: "4px 12px", borderRadius: 8, background: "rgba(16,185,129,0.12)", color: T.green, fontWeight: 700, border: `1px solid rgba(16,185,129,0.2)` }}>‚‚Äîè LIVE ¬∑ Firestore</div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        ...
        <Card label="Conversion Rate" value={stats.totalReferrals > 0 ? Math.round((stats.totalConversions / stats.totalReferrals) * 100) + "%" : "‚‚Ç¨‚Äù"} color={T.teal} sub="Referral ‚Ü‚Äô paid" />
      </div>
      ...
      <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          >
            <option value="">‚‚Ç¨‚Äù Select a user ‚‚Ç¨‚Äù</option>
          ...
          {selectedUser && (
            <div style={{ flex: 2, display: "flex", alignItems: "center", gap: 8, background: T.surfaceAlt, borderRadius: 8, border: `1px solid ${T.gold}33`, padding: "10px 14px" }}>
              ...
              <button type="button"
                onClick={() => { navigator.clipboard.writeText(getReferralLink(selectedUser)); notify("‚ú‚Ä¶ Copied to clipboard", "success"); }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
          <div style={{ fontFamily: "'Fraunces',serif", fontSize: 16, fontWeight: 700, color: T.white, marginBottom: 14 }}>üèÜ Referral Leaderboard</div>
          {loading ? (
            <div style={{ color: T.textMuted, fontSize: 13 }}>Loading...</div>
          ) : leaderboard.length === 0 ? (
            <div style={{ color: T.textMuted, fontSize: 13, textAlign: "center", padding: "20px 0" }}>No referrals yet ‚‚Ç¨‚Äù share referral links with your users to get started</div>
          ) : (
          ...
          {loading ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, maxHeight: 340, overflowY: "auto" }}>
              {referrals.slice(0, 20).map(r => {
                ...
                const statusLabel = r.status === "converted" ? "Converted ‚ú‚Äú" : r.status === "signup" ? "Signed Up" : "Clicked";
                return (
                  <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 8, background: T.surfaceAlt, border: `1px solid ${T.border}` }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: T.white, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {r.referrerEmail || "Unknown"} ‚Ü‚Äô {r.convertedEmail || r.signupEmail || "‚‚Ç¨‚Äù"}
                      </div>
                      <div style={{ fontSize: 10, color: T.textMuted }}>{r.createdAt ? new Date(r.createdAt).toLocaleDateString("en-AE") : "‚‚Ç¨‚Äù"}</div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                      ...
                      {r.rewardGranted && <span style={{ fontSize: 10, color: T.green }}>üéÅ Rewarded</span>}
                    </div>
                  </div>
                );
              })}
      ...
      <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
        ...
        <div style={{ fontSize: 12, color: T.textMuted, marginBottom: 14 }}>When a referral converts via Stripe or manually ‚‚Ç¨‚Äù log it here to trigger the reward</div>
      ...
      <div style={{ background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "20px 24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
          {[
            ...
            ["2. Click", "Friend clicks the link ‚‚Ç¨‚Äù a referral record is created in Firestore automatically", T.teal],
            ["3. Convert", "Friend signs up and upgrades to Pro ‚‚Ç¨‚Äù status updates to 'converted'", T.green],
            ["4. Reward", "Admin clicks 'Grant Month' ‚‚Ç¨‚Äù referrer gets 1 month free added to their subscription", T.gold],
          ].map(([step, desc, color]) => (
...
/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ Manual Conversion Form ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */
    ...
    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1, minWidth: 200 }}>
        ...
        <select value={referrer} onChange={e => setReferrer(e.target.value)}
          ...
          <option value="">‚‚Ç¨‚Äù Select referrer ‚‚Ç¨‚Äù</option>
...
/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ MARKET DATA EDITOR ‚‚Ç¨‚Äù S15 GAP FIX ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨
...
‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */
const MarketDataEditor = ({ db, T, notify }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      notify("‚ú‚Ä¶ Market data updated ‚‚Ç¨‚Äù dashboard will refresh within 30 seconds", "success");
    } catch(e) {
      notify("‚ùå Save failed: " + e.message, "error");
    ...
    <div style={{ marginTop: 32, background: T.surface, borderRadius: 16, border: `1px solid ${T.border}`, padding: "24px 28px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: T.textMuted, marginTop: 2 }}>Update marketData/global ‚‚Ç¨‚Äù ValuStrat ¬∑ Knight Frank ¬∑ DLD official figures</div>
        </div>
      ...
      <div style={{ padding: "10px 14px", borderRadius: 8, background: "rgba(212,168,67,0.06)", border: `1px solid rgba(212,168,67,0.15)`, fontSize: 12, color: T.textMuted, marginBottom: 16 }}>
        ‚ö†Ô∏è These values appear on the Overview, DLD Volumes, Competitors, and Market tabs. Verify against DLD official data before saving. Changes take effect immediately for all users.
      </div>
      ...
      >
        {saving ? "Saving..." : "ü‚Äôæ Save to Firestore"}
      </button>
    </div>
  );
};

/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ ICONS (matching dashboard SVG style) ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */
const I = {
...
/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ CSS (exactly matching main dashboard design DNA) ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */
const css = `
...
/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ CUSTOM TOOLTIP (matching dashboard) ‚‚Äù‚Ç¨‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */
