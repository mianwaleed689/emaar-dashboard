...
function NotificationsTab({ T, notify, adminUser, I, users, db }) {
  ...
  const [notifForm, setNotifForm] = React.useState({ title: "", message: "", icon: "ü‚Äù‚Äù", type: "info", link: "" });
  ...
  const ICONS = ["ü‚Äù‚Äù", "ü‚Äú£", "üéØ", "‚ö†Ô∏è", "üèÜ", "üåü", "ü‚Äúà", "ü‚Äùá", "‚ú‚Ä¶", "ü‚Äúä", "ü‚Äù‚Äò", "üéÅ"];
  const TYPES = [
  ...
  const sendNotification = async () => {
    ...
    try {
      ...
      setNotifForm({ title: "", message: "", icon: "ü‚Äù‚Äù", type: "info", link: "" });
  ...
  const loadTemplate = (t) => {
    setNotifForm({ title: t.title || "", message: t.message || "", icon: t.icon || "ü‚Äù‚Äù", type: t.type || "info", link: t.link || "" });
  ...
  const resendNotification = async (n) => {
    setNotifForm({ title: n.title, message: n.message, icon: n.icon || "ü‚Äù‚Äù", type: n.type || "info", link: n.link || "" });
    setTargetType("user"); setTargetUserId(n.userId);
    setNotifSubTab("compose");
    notify("Loaded for resend ‚‚Ç¨‚Äù adjust and hit Send");
  };
  ...
  const SUBTABS = [
    ...
    { id: "email",     label: "Email Blast",            icon: "ü‚Äúß" },
    { id: "scheduled", label: `Scheduled (${stats.scheduled})`, icon: "‚è∞" },
    { id: "templates", label: `Templates (${templates.length})`, icon: "ü‚Äúã" },
    { id: "history",   label: `History (${sentNotifs.length})`,  icon: "ü‚Äúú" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ COMPOSE TAB ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */}
      {notifSubTab === "compose" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: surf, borderRadius: 14, border: `1px solid ${border}`, padding: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: gold, letterSpacing: 0.5, marginBottom: 14 }}>üéØ Target Audience</div>
              ...
              <div style={{ fontSize: 11, color: blue, marginTop: 10, fontWeight: 600 }}>
                ‚Ü‚Äô {targetUsers.length} recipient{targetUsers.length !== 1 ? "s" : ""} selected
              </div>
            </div>
            ...
            <div style={{ background: surf, borderRadius: 14, border: `1px solid ${scheduleEnabled ? orange : border}`, padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: scheduleEnabled ? 14 : 0 }}>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: scheduleEnabled ? orange : white }}>‚è∞ Schedule Send</div>
            ...
            <button type="button" onClick={sendNotification} disabled={notifSending || targetUsers.length === 0}
              ...
              {notifSending ? "Sending..." : scheduleEnabled ? `‚è∞ Schedule ‚Ü‚Äô ${targetUsers.length} user${targetUsers.length !== 1 ? "s" : ""}` : `Send Now ‚Ü‚Äô ${targetUsers.length} user${targetUsers.length !== 1 ? "s" : ""}`}
            </button>
                ...
                <div style={{ fontSize: 13, color: lastResult.success ? green : red, fontWeight: 700 }}>
                  {lastResult.success
                    ? lastResult.scheduled
                      ? `‚è∞ Scheduled for ${new Date(lastResult.scheduledFor).toLocaleString("en-AE")} ‚‚Ç¨‚Äù ${lastResult.sent} users`
                      : `‚ú‚Äù Sent to ${lastResult.sent}/${lastResult.total} users in ${(lastResult.durationMs / 1000).toFixed(1)}s`
                    : `‚úò Failed: ${lastResult.error}`}
                </div>
              ...
              <div style={{ background: surfAlt, borderRadius: 12, padding: 16, border: `1px solid ${typeColor(notifForm.type)}40` }}>
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {notifForm.link && <div style={{ marginTop: 8, fontSize: 11, color: blue }}>ü‚Äù‚Äî {notifForm.link}</div>}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {templates.slice(0, 5).map(t => (
                    <button key={t.id} type="button" onClick={() => loadTemplate(t)}
                      ...
                      <span style={{ fontSize: 18 }}>{t.icon || "ü‚Äúã"}</span>
                      <div>
      ...
      {/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ EMAIL BLAST TAB ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */}
      {notifSubTab === "email" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: surf, borderRadius: 14, border: `1px solid ${border}`, padding: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: gold, marginBottom: 14 }}>üéØ Email Recipients</div>
              <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
              ...
              <div style={{ fontSize: 11, color: blue, marginTop: 10, fontWeight: 600 }}>
                ‚Ü‚Äô {getTargetUsers(emailTargetType, emailTargetTier).length} recipients
              </div>
            </div>

            <div style={{ background: surf, borderRadius: 14, border: `1px solid ${border}`, padding: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: gold, marginBottom: 14 }}>ü‚Äúß Email Content</div>
              <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ padding: "10px 14px", background: `${orange}15`, borderRadius: 8, border: `1px solid ${orange}40`, marginBottom: 14 }}>
                <div style={{ fontSize: 10, color: orange, fontWeight: 600 }}>‚ö†Ô∏è Note: Using EmailJS ‚‚Ç¨‚Äù max ~200 emails/month on free plan. Upgrade at emailjs.com for higher volume.</div>
              </div>
              <button type="button" onClick={sendEmail} disabled={emailSending}
                ...
                {emailSending ? "Sending emails..." : `ü‚Äúß Send Email to ${getTargetUsers(emailTargetType, emailTargetTier).length} Users`}
              </button>
            </div>
          </div>
      ...
      {/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ SCHEDULED TAB ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */}
                ...
                <div key={n.id} style={{ padding: "16px 20px", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 10, color: orange, marginTop: 4, fontWeight: 600 }}>
                      ‚è∞ Scheduled: {new Date(n.scheduledFor).toLocaleString("en-AE")} ¬∑ To: {n.userName || n.targetType}
                    </div>
                  </div>
      ...
      {/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ TEMPLATES TAB ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */}
      {notifSubTab === "templates" && (
        <div style={{ background: surf, borderRadius: 14, border: `1px solid ${border}`, overflow: "hidden" }}>
          <div style={{ padding: "16px 20px", borderBottom: `1px solid ${border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button type="button" onClick={() => { setNotifForm({ title: "", message: "", icon: "ü‚Äù‚Äù", type: "info", link: "" }); setNotifSubTab("compose"); setShowTemplateModal(true); }}
              style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: gold, color: bg, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              + New Template
            </button>
          </div>
          {templates.length === 0 ? (
            <div style={{ padding: 60, textAlign: "center", color: textMuted }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>ü‚Äúã</div>
                ...
                <div key={t.id} style={{ background: surfAlt, borderRadius: 12, border: `1px solid ${border}`, padding: 16 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontSize: 24 }}>{t.icon || "ü‚Äúã"}</span>
                      <div>
      ...
      {/* ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ HISTORY TAB ‚‚Äù‚Ç¨‚‚Äù‚Ç¨ */}
      {notifSubTab === "history" && (
        <div style={{ background: surf, borderRadius: 14, border: `1px solid ${border}`, overflow: "hidden" }}>
          ...
          {filteredHistory.length === 0 ? (
            <div style={{ padding: 60, textAlign: "center", color: textMuted }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>ü‚Äú°</div>
...
/* ===================================================================
   TAB 11: EMAIL DIGEST COMPONENT ‚‚Ç¨‚Äù PRO LEVEL
