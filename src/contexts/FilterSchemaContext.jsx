/* eslint-disable */
/**
 * DXB Analytics ââ‚¬â€ FilterSchemaContext
 * =====================================
 *
...
export function FilterSchemaProvider({ children }) {
  ...
  const derived = useMemo(() => {
    ...
    const allTypeValues = schema.propertyTypes
      .flatMap(g => g.types || [])
      .map(t => t.value);
    // Type value â†â€™ label map (for filter matching with mixed casing)
...
export function useFilterSchema() {
  ...
  if (!ctx) {
    // Graceful fallback ââ‚¬â€ if a component is rendered outside the provider,
